...
class Html2Pdf
{
    ...
    protected $_imageInfosCache  = array();     // image size cache
    ...
    /**
     * Get image size with a per-document cache.
     *
     * @param string $src
     *
     * @return array|false
     */
    protected function getImageInfos($src)
    {
        $cacheKey = (string) $src;
        if (array_key_exists($cacheKey, $this->_imageInfosCache)) {
            return $this->_imageInfosCache[$cacheKey];
        }

        $this->security->checkValidPath($cacheKey);
        $this->_imageInfosCache[$cacheKey] = @getimagesize($cacheKey);
...
<html>
    ...
    protected function _drawImage($src, $subLi = false)
    {
        ...
        if (strpos($src,'data:') === 0) {
            ...
            $infos = $this->getImageInfos((string) $src);
        }
    ...
    protected function _drawRectangle($x, $y, $w, $h, $border, $padding, $margin, $background)
    {
        ...
        if ($background['image']) {
            ...
            $imageInfos = $this->getImageInfos((string) $iName);
            ...
            if (isset($param['backimg'])) {
                ...
                if ($background['img']) {
                    ...
                    if( strpos($background['img'],'data:') === 0 ) {
                        ...
                        $infos = $this->getImageInfos((string) $background['img']);
                    }
