...
class Html
{
    ...
    /**
     * Tags that were closed implicitly to recover browser-tolerated HTML.
     * @var int[]
     */
    protected $implicitlyClosedTags = array();
    ...
    public function parse($tokens)
    {
        ...
        $this->tagPreIn = false;
        $this->implicitlyClosedTags = array();
    ...
    protected function getTagAction(Token $token, &$parents)
    {
        ...
        $tableClosingTags = array('table', 'thead', 'tfoot', 'tbody', 'tr', 'td', 'th');
        ...
        if (!in_array($node->getName(), $tagsNotClosed)) {
            ...
            if ($node->isClose()) {
                // HTML validation
                if (count($parents) < 1) {
                    if (!empty($this->implicitlyClosedTags[$node->getName()])) {
                        $this->implicitlyClosedTags[$node->getName()]--;
                        return array();
                    }
                    if (in_array($node->getName(), $tableClosingTags, true)) {
                        return array();
                    }
                    ...
                    $parentKey = array_search($node->getName(), array_reverse($parents, true), true);
                    if ($parentKey !== false) {
                        while (end($parents) != $node->getName()) {
                            $closedTag = array_pop($parents);
                            if (!isset($this->implicitlyClosedTags[$closedTag])) {
                                $this->implicitlyClosedTags[$closedTag] = 0;
                            }
                            $this->implicitlyClosedTags[$closedTag]++;
                            $implicitNode = new Node($closedTag, array(), true);
                            $implicitNode->setLine($token->getLine());
                            $actions[] = $implicitNode;
                        }
                        array_pop($parents);
                    } elseif (!empty($this->implicitlyClosedTags[$node->getName()])) {
                        $this->implicitlyClosedTags[$node->getName()]--;
                        return array();
                    } elseif (in_array($node->getName(), $tableClosingTags, true)) {
                        return array();
                    } else {
