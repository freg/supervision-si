...
function oppositeColor(hex, bw) {
    ...
    return "#" + r.padZero(2) + g.padZero(2) + b.padZero(2);
}
...
function fillLeavesCalendarDataArray(startDate,endDate){
    ...
    idEvent=0;
    dojo.xhrGet({
        url : "../tool/getLeaves.php?startDate="+startDate+"&endDate="+endDate+"&idRes="+idRes+addTokenIndexToUrl(),
        handleAs : "text",
        sync: true,
        load : function(data, args) {
...
function refreshEmployeeCustomLeavesEarned(){
    ...
    dojo.xhrGet({
        url : "../tool/getEmployeeCustomLeavesEarned.php?idEmployee="+idEmployee+addTokenIndexToUrl(),
        handleAs : "text",
        sync: true,
        load : function(data, args) {
...
function refreshEmployeeLeavesSummary(){
    ...
    //return lvType:(idLeaveType/color),lvTypeOfEmpContractType:(periodDuration),lvEarned:(startDate/endDate/quantity/leftQuantity), 
    dojo.xhrGet({
        url : "../tool/getEmployeeLeaveEarnedSummary.php?idEmployee="+idEmployee+addTokenIndexToUrl(),
        handleAs : "text",
        sync: true,
        load : function(data, args) {
            var lines = JSON.parse(data);
            ...
            for(var i=0;i<len;i++){
               ...
               summaryContent += '<tr style="border: solid 1pt; height: 20px;'+((lines[i].idle=='1')?'background-color:#c0c0c0;':'')+'">';
...
function deletePopupAction(){
    ...
    if(creatBool === false){
       dojo.xhrGet({
            url : "../tool/deleteLeaveOfCalendar.php?idLeave="+dijit.byId("popupLeaveId").value+addTokenIndexToUrl(),
            handleAs : "text",
            sync: true,
            load : function(data, args) {
                var res=JSON.parse(data);
...
function validOrCancelLeave(idLeave,motif,context) {
    ...
    if (context!="CAL") {
        ...
        // CAL : popup opened from the calendar. The user clicked a submitted leave to act on it,
        // so the most likely intent is "accept". Preselect the accept status but keep the select
        // enabled so the user can switch to reject (or any other allowed status) if needed.
        // Without this, the select keeps its previous/default value (often the leave's current
        // "submitted" status), so clicking Valider without changing it saved nothing -> NO_CHANGE.
...
function getLeftByLeaveType(idEmployee) {
    ...
    dojo.xhrGet({
        url : "../tool/getLeftQuantityByLeaveTypeForAnEmployee?idEmployee="+idEmployee+addTokenIndexToUrl(),
        handleAs : "text",
        sync: true,
        load : function(data, args) {
...
function moveSkillFromHierarchicalView (idFrom, idTo){
  ...
  var url='../tool/moveSkillFromHierarchicalView.php?idFrom=' + idFrom
      + '&idTo=' + idTo + '&mode=' + mode+addTokenIndexToUrl();
  dojo.xhrPost({
    url : url,
    handleAs : "text",
    load : function() {
