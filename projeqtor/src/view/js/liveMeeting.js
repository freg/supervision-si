...
var timerBeforeSaveValue=30;
var timerBeforeSave=timerBeforeSaveValue; //save toutes les 60 secondes
...
function seekWhoSpeak(){
  ...
  if(exist){
    ...
    if(timerBeforeSave<=0){
      liveMeetingSave();
      timerBeforeSave=timerBeforeSaveValue;
    }
  }
}
...
function liveMeetingSave(noHistory){
  ...
  dojo.xhrPost({
    ...
    +"&idSpeaker="+allwhoSpeak+"&allOrganizator="+allOrganizator+"&allCanSpeak="+allCanSpeak+"&idLiveMeeting="+dojo.byId("liveMeetingId").value+"&noHistory="+(noHistory ? 1 : 0)+addTokenIndexToUrl(),
...
function liveMeetingAddToEditor(toPass, kanbanChange){
  var time = new Date();
  var now="";
  var nowFmt="";
  ...
  if (editorType=="CK") { // CKeditor type
    ...
    } else if (kanbanChange == true) {
      valTmp+='<div> ' +toPass + '&nbsp;&nbsp;</div>';
    } else {
...
function liveMeetingGoPlay(){
  ...
  if(dijit.byId('playPauseButton').get('iconClass').indexOf('iconLiveMeetingPlay')!=-1){
    dijit.byId('playPauseButton').set('iconClass','iconLiveMeetingPause22');
    ...
    if (dojo.byId('nameFor'+idFirst)) liveMeetingStartTimerSpeak('blockFor'+idFirst,dojo.byId('nameFor'+idFirst).innerHTML);
  }else{
    if(idFirst==-1 && whoSpeakLast!=-1)idFirst=whoSpeakLast;
    dijit.byId('playPauseButton').set('iconClass','iconLiveMeetingPlay22');
    if (dojo.byId('a'+whoSpeak)) liveMeetingStartTimerSpeak('blockFor'+whoSpeak,dojo.byId('a'+whoSpeak).innerHTML);
  }
}
...
function liveMeetingResizeEditor(loopCount) {
  ...
  if (editorType=="CK") { // CKeditor type
    ...
    if (editor.status=='ready') {
      ...
      if (dojo.byId("divKanbanAllContainer")) {
        var height=dojo.byId("divKanbanAllContainer").offsetHeight; 
        height-=(dijit.byId('kanbanSeeWork').get("value")=='on')?198:166;
        dojo.query(".kanbanColumn.dojoDndSource.dojoDndTarget").forEach(function(domNode) {
          domNode.style.height=(height)+"px";
          domNode.style.maxHeight=(height)+"px";
        });
      } else if (dojo.byId("divBottom")) {
        var height=dojo.byId("divBottom").offsetHeight; 
        dojo.query(".liveMeetingBottomDivs").forEach(function(domNode) {
          domNode.style.height=(height-68)+"px";
          domNode.style.maxHeight=(height-68)+"px";
        });
      }
    } else {
...
function addNewLinkMeeting (item,comboName,idProj){
  ...
  if(objName && objClass){
    ...
    showWait();
    dojo.xhrPost({
          url : "../tool/saveLiveMeetingAttachment.php?objectClass="+objClass+'&objName='+objName+'&idProj='+idProj+addTokenIndexToUrl(),
          form :null,
          handleAs : "text",
          load : function(data, args) {
            ...
            hideWait();
            focusOnInputAfterAddNewLinkMeetingCount=0;
            setTimeout("focusOnInputAfterAddNewLinkMeeting('"+item+"')",100);
          },
          error : function() {
            hideWait();
          }
...
focusOnInputAfterAddNewLinkMeetingCount=0;
function focusOnInputAfterAddNewLinkMeeting(item) {
  dijit.byId(item).focus();
  liveMeetingResizeEditor();
  focusOnInputAfterAddNewLinkMeetingCount++;
  if (focusOnInputAfterAddNewLinkMeetingCount<10) setTimeout("focusOnInputAfterAddNewLinkMeeting('"+item+"')",100);
}
