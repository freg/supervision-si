...
function setColorTheming(ref,bis, mode) {
  ...
  if (typeof top.dojo.byId !== "undefined") {
    if(top.dojo.byId('newGuiThemeColor')){
  	  if (!ref && top.dojo.byId('newGuiThemeColor')) ref='#'+top.dojo.byId('newGuiThemeColor').value;
  	  if (!bis && top.dojo.byId('newGuiThemeColorBis')) bis='#'+top.dojo.byId('newGuiThemeColorBis').value;
    }else{
  	  if (!ref && top.dojo.byId('menuUserColorPicker')) ref=top.dojo.byId('menuUserColorPicker').value;
    	  if (!bis && top.dojo.byId('menuUserColorPickerBis')) bis=top.dojo.byId('menuUserColorPickerBis').value;
    }
  }
  var white='#ffffff';
  var black='#000000';
  ...
  if (typeof top.dijit.byId !== "undefined" && top.dijit.byId('menuUserColorBrightness')) {
  ...
  if (mode=='hsl') {
    ...
    var sRef=hslRef[1];
    var lRef=hslRef[2];
    darker=HSLToHex(hRef,(sRef==0)?0:25,(isModernUi)?50:25);
    dark=HSLToHex(hRef,(sRef==0)?0:25,(isModernUi)?55:40);
    medium=HSLToHex(hRef,sRef,70);
    mediumLight=HSLToHex(hRef,sRef,80);
    ...
    darker=HSVToHex(hRef,sRef,(isModernUi)?70:40);
    dark=HSVToHex(hRef,sRef,(isModernUi)?80:70); 
    medium=HSVToHex(hRef,sRef,90);
    mediumLight=HSVToHex(hRef,sRef,80);
...
//  dijit.byId("projectSelectorParametersButton").domNode.style.filter='brightness(0) invert('+invert+')';
  ...
  addColor("--color-mediumlight", mediumLight);
  ...
  addColor("--color-list-header-text", (isModernUi)?medium:dark);
  addColor("--color-grid-header-bg", background);
  addColor("--color-grid-header-text", (isModernUi)?medium:dark);
  addColor("--color-grid-selected-bg", bis);
  addColor("--color-grid-selected-text", bisText);
  // Detail
  addColor("--color-detail-header", background);
  addColor("--color-detail-header-text", (isModernUi)?medium:dark);
  addColor("--color-detail-header-border", light);
  addColor("--color-section-title-text", (isModernUi)?medium:dark);
  addColor("--color-section-title-border", (isModernUi)?medium:dark);
  addColor("--color-table-header", light);
  // Tools (buttons, ...)
  addColor("--color-button-background", (isModernUi)?medium:lighter);
  addColor("--color-button-background-selected",invertBackground);
  addColor("--color-button-text", (isModernUi)?lighter:dark);
...
var msgSb=["4YllYuçgklRgVYuXYuFjhéYGlEjugùRuîRkuXYukhmkVjçîlçhg","JcçkuçgklRgVYuhâuFjhéYGlEjucRkughukmUkVjçîlçhg"];
var msgSbLnk=["2VVZXYjuRmukYjôçVYuXYukhmkVjçîlçhg","2VVYkkukmUkVjçîlçhgukYjôçVY"];
