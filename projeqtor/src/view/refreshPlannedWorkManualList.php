...
$idProject = pq_trim(RequestHandler::getId('idProjectPlannedInt'));
$yearSpinner= RequestHandler::getYear('yearPlannedWorkManual');
$monthSpinner= RequestHandler::getMonth('monthPlannedWorkManual');
$showDonePlannedWork=(pq_trim(Parameter::getUserParameter('showDonePlannedWork'))!='0')?true:false;
$displayNothing = false;$onlyRes=false;
$listResource = array();
$noNeed = false;
$resourceId = pq_trim(RequestHandler::getId('userNamePlanned'));
$inIdTeam = pq_trim(RequestHandler::getId('idTeamPlannedWorkManual'));
$inIdOrga = pq_trim(RequestHandler::getId('idOrganizationPlannedWorkManual'));
  ...
  <div dojoType="dijit.layout.BorderContainer" >
    ...
    <div  dojoType="dijit.layout.ContentPane" region="center" style="overflow:auto" id="planningManualBottomArea" onscroll="dojo.byId('planningManualTopArea').scrollLeft=(this.scrollLeft);">
      ...
      <div id="plannedWorkManualInterventionDiv"  name="plannedWorkManualInterventionDiv" style="min-width:1123px;left:<?php echo (isNewGui())?485:474;?>px;top:20px;position:absolute;">
              <?php //TAB RESOURCES
              if (!$yearSpinner or $yearSpinner<2000) $yearSpinner=date('Y');
              if (!$monthSpinner or $monthSpinner<1 or $monthSpinner>12) $monthSpinner=date('m');
