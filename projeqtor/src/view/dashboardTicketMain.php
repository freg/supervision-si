 ...
 */
 ...
 //FLORENT
  ...
  if (Parameter::getUserParameter("dashboardTicketMainTypes")) {
    ...
    $filterTypesArray = pq_explode(', ', $filterTypes);
  }
  $defaultProject = null;
  ...
  if ($filterTypes == '' and pq_array_key_exists('dashboardTicketMainType', $_REQUEST)) {
...
<div style="width: 97%; margin: 0 auto; height: 90px; padding-bottom: 15px; border-bottom: 1px solid #CCC;">
			<table width="100%" class="dashboardTicketMain">
				...
				<td valign="top" style="width:30%">
						<table>
						<tr height="37px"><td>&nbsp;&nbsp;<?php echo pq_ucfirst(i18n('filterByTicket'));?></td> </tr>
								<tr>
								<?php 
                      ...
                      <li onClick="changeParamDashboardTicket('dashboardTicketMainAllTicket=1')" data-dojo-type="dojox/mobile/TabBarButton" <?php if($paramDashboardTicketMainAllTicket==1){ echo "data-dojo-props='selected:true'"; }?> ><?php echo i18n('Unresolved');?></li>
  				        ...
  				        if(Parameter::getUserParameter("dashboardTicketMainUnresolved")!=null){
  				          $dashboardTicketMainUnresolved=Parameter::getUserParameter("dashboardTicketMainUnresolved");
								  ...
								  <td>
								    <ul style="top:-8px;" data-dojo-type="dojox/mobile/TabBar" data-dojo-props='barType:"segmentedControl"'>
                      <li onClick="changeParamDashboardTicket('dashboardTicketMainUnresolved=1')" data-dojo-type="dojox/mobile/TabBarButton" <?php if($dashboardTicketMainUnresolved==1){ echo "data-dojo-props='selected:true'"; }?> ><?php echo pq_ucfirst(i18n("dashboardRequirementMainUnscheduled"));?></li>
                    </ul>
								  </td>
								</tr>
						</table>
...
function addTab($param){
  ...
  if ($total > 0) {
    ...
    ksort($res);
    echo '<h2 style="color:#333333;font-size:16px;">'.pq_trim(i18n("dashboardTicketMainTitleBase"))." ".(i18n($param["title"])).$addIfNoParam."</h2>";
    ...
    echo '<h2 style="color:#333333;font-size:16px;">'.(pq_substr(i18n("dashboardTicketMainTitleBase"),-1)==" "?i18n("dashboardTicketMainTitleBase"):i18n("dashboardTicketMainTitleBase")." ").(i18n($param["title"]))."</h2>";
...
function createPopUpDnd($tabPosition){
    ...
    if(isNewGui()){
      ...
      echo pq_ucfirst(i18n("filterSynthesis"));
      echo'   </td>';
      $valueSwitch = "off";
      if(Parameter::getUserParameter("filterSynthesis")!=null){
...
foreach ($tabPosition['orderListLeft'] as $tableauBordLeftItem) {
  ...
  echo "<td valign=\"top\" style=\"padding-right:10px;\"><span class='nobr'>".(pq_substr(i18n("dashboardTicketMainTitleBase"),-1)==" "?i18n("dashboardTicketMainTitleBase"):i18n("dashboardTicketMainTitleBase")." ").(i18n($tabPosition[$tableauBordLeftItem]['title']))."</span>";
  echo '</td>';
  echo '</tr>';
}
...
foreach ($tabPosition['orderListRight'] as $tableauBordRightItem) {
  ...
  echo "<td valign=\"top\" style=\"padding-right:10px;\"><span class='nobr'>".(pq_substr(i18n("dashboardTicketMainTitleBase"),-1)==" "?i18n("dashboardTicketMainTitleBase"):i18n("dashboardTicketMainTitleBase")." ").(i18n($tabPosition[$tableauBordRightItem]['title']))."</span>";
  echo '</td>';
  echo '</tr>';
}
echo '</table></td></tr>
...
function addParamToUser($user){
  ...
  if($tabPosition[$objectClass]["withParam"]){
    ...
    $recent=Parameter::getUserParameter("dashboardTicketMainRecent");
    $nbDay=Parameter::getUserParameter("dashboardTicketMainNumberDay");
    if (preg_match('/[^\-0-9]/', pq_nvl($nbDay)) == true) {
      $nbDay="";
    }
    if($recent=="1"){
