...
<div class="swapView" data-dojo-type="dojox/mobile/SwapView"  id="divNewsPage1" name="divNewsPage1">
        <table>
          <tr><?php 
            ...
            $urlGetNews.="?source=refreshLastNews&".RequestHandler::getNewsInfo();
            $json=null;
...
//               $ctx=stream_context_create(array('http'=>$http_options));
              ...
              $json = file_get_contents($urlGetNews,false,$ctx);
              disableCatchErrors();
              disableSilentErrors();
           ...
           $i=1;
           if (!$obj) {
             echo '<td style="margin:5px;color:#505050;font-style:italic;text-align:justify">'.i18n('noInternetAccess').'</td>';
           } else if ($obj) foreach ($obj as $objV=>$val){
              if($objV=='items'){
                foreach ($val as $value){
                 ...
                 if($i==3){?><tr><?php } 
                   ...
                   if (pq_strpos($valueFullText,$stringScr) !== FALSE) {
                      $valueFullText = pq_str_replace($stringScr, $stringNewScr, $valueFullText);
                   }
                   if (pq_strpos($valueIntrotext,$stringHref) !== FALSE) {
                     $valueIntrotext = pq_str_replace($stringHref, $stringNewHref, $valueIntrotext);
                   }
                   if (pq_strpos($valueFullText,$stringHref) !== FALSE) {
                     $valueFullText = pq_str_replace($stringHref, $stringNewHref, $valueFullText);
                   }
                 ?>
                <td>
                  <table>
    ...
    <div class="swapView" data-dojo-type="dojox/mobile/SwapView"  id="divNewsPage2" name="divNewsPage2">
        <table>
          <tr><?php
          $i=0;
          if ($obj) foreach ($obj as $objV=>$val){
              if($objV=='items'){
                foreach ($val as $value){
                 if($value->lang!=$lang )continue;
                 $i++;
