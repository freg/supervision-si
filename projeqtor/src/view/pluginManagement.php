              ...
              <tr height="30px"> 
                ...
                <td style="text-align:left;width;200px" >
                 ...
                 <div MAX_FILE_SIZE="<?php echo Parameter::getGlobalParameter('paramAttachmentMaxSize');?>"
                  ...
                  url="../tool/uploadPlugin.php<?php echo Security::addTokenIndexToUrl('?');?>"
...
function displayInstalledPlugin() {
  ...
  if (count($plList)==0) {
    ...
    foreach ($plList as $plugin) {
      $canDelete=true;
      if (pq_substr($plugin->name,0,11)=='pluginPatch') $canDelete=false;
      echo '<tr>';
      echo '<td class="noteData">';
      if ($canDelete) echo '  <a onClick="uninstallPlugin(\''.$plugin->uniqueCode.'\', \''.$plugin->name.'\');" title="' . i18n('buttonDeletePluginFile') . '"  /> '.formatSmallButton('Remove').'</a>';
      echo '</td>';
