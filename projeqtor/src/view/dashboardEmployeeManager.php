 ...
 */
  ...
  $idEmployeeRequest=0;
  if (pq_array_key_exists("idEmployee", $_REQUEST)) {
      $idEmployeeRequest = $_REQUEST['idEmployee'];
  }
  // Whether the connected user is allowed to see every employee's absences (visibility only)
  $canSeeAll = canSeeAllAbsences($user);
  // List of employees : all employees if "see all" is granted, else only managed employees
  if ($canSeeAll) {
  ...
  $idStatusRequest=0;
  if (pq_array_key_exists("idStatus", $_REQUEST)) {
  ...
  if (pq_array_key_exists("year", $_REQUEST)) {
      $yearRequest = $_REQUEST['year'];
  }
  $monthRequest=(new DateTime())->format("m");
  if (pq_array_key_exists("month", $_REQUEST)) {
        ...
        <table style="width:100%; height:90%;text-align:center;border-top:1px solid black;">
            <tr style="height:100%;width:100%;vertical-align:top;">
                ...
                <td style="width:60%;">
                    ...
                    <!-- CALENDAR CONTENT -->
                    <!---------------------->
                    <table style="margin-left:20px;vertical-align:top;text-align:center;margin-right:20px;">
                        <!------------>
                        <!-- HEADER -->
                        <!------------>
                        ...
                        <!----------->                        
                            <?php
                                ...
                                foreach($employeeListRequest as $key => $name) {
                                    ...
                                    // The connected user can validate this employee's leaves only if leaves admin
                                    // or actual manager of this employee. Drives display of the click-to-validate
                                    // affordance on submitted leaves in the calendar.
                                    $canValidateThisEmployee = (isLeavesAdmin(getSessionUser()->id)
                                        || isManagerOfEmployee(getSessionUser()->id, $employee->id));
                                    ...
                                    echo '  <td class="assignHeader"><div class="dashboardTdName" title="'.$name.'">'.$showName.'</di></td>';
                                    for($i=1;$i<=31;$i++) {
                                        ...
                                        $idStatusPM = 0;
                                        if ($leavesDay!=null) { 
                                            if (pq_array_key_exists($date, $leavesDay)) {
                                                ...
                                                if(pq_array_key_exists('idTypeAM', $leavesDay[$date])){
                                                  $idTypeAM = ($isOffDay?0:$leavesDay[$date]['idTypeAM']);
                                                }
                                                if(pq_array_key_exists('idTypePM', $leavesDay[$date])){
                                                  $idTypePM = ($isOffDay?0:$leavesDay[$date]['idTypePM']);
                                                }
                                                
                                                if(pq_array_key_exists('idStatusAM', $leavesDay[$date])){
                                                  $idStatusAM = ($isOffDay?0:$leavesDay[$date]['idStatusAM']);
                                                }
                                                if(pq_array_key_exists('idStatusPM', $leavesDay[$date])){
                                                ...
                                                if ($submitted==0 and $rejected==0 and $accepted==0) {
                                                    if ($lvTpHasWfStWithSubmitted==null or !pq_array_key_exists($idLeaveType, $lvTpHasWfStWithSubmitted)) {
                                                ...
                                                if($idStatusAM!=0 or $idStatusPM!=0){
                                                  ...
                                                  if (pq_array_key_exists($leavesDay[$date]['idStatus'], $leaveStatusColor)) {
                                                ...
                                                if ($leavesDay[$date]['AM']) {
                                                    if (pq_array_key_exists($leavesDay[$date]['idStatus'], $statusListFirstLetter)) {
                                        ...
                                        if ($i>$lastDayOfMonth) {
                                            ...
                                            if ($idLeave) {
                                                ...
                                                if ($statusOutOfWorkflow==1 or $statusSetLeaveChange==1 ) {
                                                    ...
                                                    if ($submitted==1 and $canValidateThisEmployee) {
                                                        $title .= i18n("clickToValidOrCanceled");
                                                        $title .= "\r".' Date = '.$leaveStartDate;
                                                        $title .= ' -- '.$leaveEndDate;
                                                        $onClick='onClick="getWorkflowStatusesOfLeaveType(null,'.$idLeaveType.','.$employee->id.');validOrCancelLeave('.$idLeave.',\''.$motif.'\');"';
                                                        $cursor = 'cursor:pointer;';
                                                    }
                                ...
                                <table style="width:100%;height:90%;border:1px solid">
                                    ...
                                    <!--------------------->                                        
                                        <?php
                                            ...
                                            foreach($employeeListRequest as $id => $name) {
                                                if ( ! $canSeeAll and ! isLeavesManager(getSessionUser()->id) and $id!=$user->id) {
                                                  continue;
                                                }
                                                $emp = $employees[$id];
                                                ...
                                                if ($idTypeRequest==0) {
                                                    ...
                                                    foreach($leaveTypes as $leaveType) {
                                                        ...
                                                        if (pq_array_key_exists($leaveType->id, $leftByType)) {
                                                    ...
                                                    $left=null;
                                                    if (pq_array_key_exists($idTypeRequest, $leftByType)) {
                                ...
                                <table style="width:100%;height:90%;">
                                    ...
                                    <?php
                                        ...
                                        foreach($employees as $employee) {
                                            if ( ! $canSeeAll
                                                 and ! isLeavesManager(getSessionUser()->id)
                                                 and $employee->id != $user->id) {
                                                continue;
                                            }
                                            $canValidateThisEmployee = (isLeavesAdmin(getSessionUser()->id)
                                                || isManagerOfEmployee(getSessionUser()->id, $employee->id));
                                            ...
                                            foreach ($lvList as $leave) {
                                                if ($lvTpHasWfStWithSubmitted==null or !pq_array_key_exists($leave->idLeaveType, $lvTpHasWfStWithSubmitted)) {
                                                ...
                                                if ($leave->statusOutOfWorkflow==1 or $leave->statusSetLeaveChange==1) {
                                                    ...
                                                    echo '<tr>';
                                                    if ($canValidateThisEmployee) {
                                                        echo '  <td class="assignHeader" style="text-align:center;vertical-align:middle;">';
                                                        ...
                                                        if ($canValidateThisEmployee) {
                                                            ...
                                                            foreach($listStatus as $key=>$status) {
                                                                if ($status->setRejectedLeave==1 or $status->setAcceptedLeave==1) {
                                                                    $onClick='onClick="getWorkflowStatusesOfLeaveType(null,'.$leave->idLeaveType.','.$employee->id.');validOrCancelLeave('.$leave->id.',\''.$leave->comment.'\',\''.($status->setRejectedLeave==1?'CAN':'VAL').'\');"';
                                                                    $textColor = oppositeColor($status->color);
                                                ...
                                                echo '  <td '.($canValidateThisEmployee?'':'colspan="2" ').'style="text-align:center;vertical-align:middle;" class="assignHeader">';
                                                echo $employee->name;
                                                echo '  </td>';
                                                // Type                                                
        ...
        <tr>
            ...
            <td>
                <select id='popupLeaveStatus' dojoType='dijit.form.Select' required>
                    <?php
                    foreach($listStatus as $status) {
                        if ($status->id==1 or $status->setSubmittedLeave==1) {continue;}
                        echo '<option value="' . $status->id . '"';
                         echo '><span >'. htmlEncode($status->name) . '</span></option>';
                    }
                    ?>
