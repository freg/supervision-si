            ...
            <tr class="detail">
              ...
              <td>
                ...
                <?php echo autoOpenFilteringSelect();?>
                 ...
                 $critFld=null;
                 $critVal=null;
                 if (sessionValueExists('project') and getSessionValue('project') and getSessionValue('project')!='*' and pq_strpos(getSessionValue('project'), ",") === null) {
                 	$critFld='idProject';
...
// MTY - LEAVE SYSTEM
       ...
       // isEmployee
            ...
            if (property_exists($obj,'idWorkflow') and  pq_strpos($isType,'Type')==(pq_strlen($isType)-4)) {
            ?>
                <tr class="detail">
...
function isDisplayable($obj, $field, $fromPlanningElement=false) {
  ...
  and ! in_array($field,$extraHiddenFields) and ! in_array($field,$extraReadonlyFields)) {
    ...
    if ($fromPlanningElement and property_exists($obj,$pe) and is_object($obj->$pe) and property_exists($obj->$pe,$field)) {
      ...
      if ( (SqlElement::isAmount($field) and $costVisibility!='ALL') or (SqlElement::isWork($field) and $workVisibility!='ALL') ) {
        return false;
      }
