...
$userTimeZone = getSessionValue('timeZone');
if (!$userTimeZone and isset($timeZone)) $userTimeZone=$timeZone;
...
if (securityCheckDisplayMenu(null,'ProjectDashboard')) {$listStartPage['projectDashboardMain.php']=i18n('menuProjectDashboard');}
...
$welcome = $listStartPage['welcome.php'];
unset($listStartPage['welcome.php']);
asort($listStartPage, SORT_NATURAL | SORT_FLAG_CASE);
$listStartPage = ['welcome.php' => $welcome] + $listStartPage;

$menu=SqlElement::getSingleSqlElementFromCriteria('Menu', array('name'=>'menuUserParameter'));
$showUserParameters=securityCheckDisplayMenu($menu->id,pq_substr($menu->name,4));
?>
  ...
  <tr style="height:40px" class="menuUserTopDetail" >
    <td <?php if ($showUserParameters) echo'rowspan="2"';?> style="white-space:nowrap;vertical-align:middle;text-align:center;position:relative;"><?php if ($imgUrl) { echo '<img style="border-radius:40px;height:80px" src="'.$imgUrl.'" />'; } else { ?>
            <div style="overflow-x:hidden;position: relative; width:80px;height:80px;border-radius:40px; border: 1px solid grey;color: grey;font-size:80%; text-align:center;cursor: pointer;" 
              onClick="addAttachmentProfilePic(<?php echo getCurrentUserId()?>);" title="<?php echo i18n('addPhoto');?> "><div style="font-size:80%;position:relative;top:32px"><?php echo i18n('addPhoto');?></div></div> 
   <?php } ?></td>
   <td>
  ...
  <tr>
    <td style="white-space:nowrap;vertical-align:middle;text-align:center;position:relative;"><?php if ($imgUrl) { echo '<img style="border-radius:40px;height:80px" src="'.$imgUrl.'" />'; } else { ?>
            <div style="overflow-x:hidden;position: relative; width:60px;height:60px;border-radius:40px; border: 1px solid grey;color: grey;font-size:80%; text-align:center;cursor: pointer;" 
              onClick="addAttachmentProfilePic(<?php echo getCurrentUserId()?>);" title="<?php echo i18n('addPhoto');?> "><div style="left: 19px;position:relative;top: 20px;height:22px;width: 22px;" class="iconAdd iconSize22 imageColorNewGui">&nbsp;</div></div> 
   <?php } ?>
   </td>
   <td style="padding-left: 10px;">
    <table>
      <tr>
        <td style="font-weight:bold;font-size: 12pt;color: var(--color-dark);white-space:nowrap;"><?php echo pq_ucfirst(($user->resourceName)?$user->resourceName:$user->name);?></td>
      </tr>
      <tr>
...
<?php   $listValues=Parameter::getList('lang');
  ...
  <tr style="height:40px">
    <td width="120px" style="text-align:right;position:relative;"><div style="white-space:nowrap;position:absolute;right:0px;top:12px"><?php echo i18n("paramTimeZone");?>&nbsp;<?php if (! isNewGui()) echo ":&nbsp;"; ?></div></td>
    <td>  
      <select dojoType="dijit.form.FilteringSelect" class="input" name="TimeZoneMenuUserTop" id="TimeZoneMenuUserTop" 
        <?php echo autoOpenFilteringSelect();?>
        title="<?php echo i18n('helpParamDefaultTimezone');?>" style="width:225px">
        <script type="dojo/connect" event="onChange" >
          changeTimeZone(this.value);
        </script>
<?php   $listValues=Parameter::getTimezoneList();
        foreach ($listValues as $value => $valueLabel ) {
          $selected = ($userTimeZone==$value)?'selected':'';
          $value=pq_str_replace(',','#comma#',$value); // Comma sets an isse (not selected) when in value
          echo '<option value="' . $value . '" ' . $selected . '>' . $valueLabel . '</option>';
        }?>
      </select>
...
<?php }?>
<?php if (Parameter::getGlobalParameter('startPageLocked')!='YES') {?>
  <tr style="height:40px">
...
<?php }?>
  ...
  <?php  
   ...
   $refreshAutoTimer=(Parameter::getUserParameter('refreshAutoTimer')!='')?Parameter::getUserParameter('refreshAutoTimer'):30;
   if(!$refreshAutoTimer or $refreshAutoTimer=='')$refreshAutoTimer=30;
   $disabled=($refreshAuto)?'disabled':'';?>
   <tr style="height:40px">
    <td width="120px" style="text-align:right"><?php echo i18n("refreshAuto");?></td>
    <td>
       <input id="refreshAuto" name="refreshAuto" type="hidden" value="<?php echo $refreshAuto;?>" />
       <div align="center">
         <table>
          <tr>
            <td>
            <div title="<?php echo i18n('helpRefreshAuto');?>" id="runRefreshAuto" name="runRefreshAuto" class="colorSwitch" data-dojo-type="dojox/mobile/Switch" value="<?php if($refreshAuto){?>on<?php }else{?>off<?php }?>" leftLabel="" rightLabel="">
              <script type="dojo/method" event="onStateChanged" >
                setRefreshAuto();
              </script>
            </div>
            ...
            <td style="padding-left:10px;">
              <div <?php echo $disabled;?> id="refreshAutoTimer" dojoType="dijit.form.NumberTextBox" class="input" constraints="{min:5, max:300}" style="width:50px;" value="<?php echo $refreshAutoTimer;?>" ></div>s
            </td>
          </tr>
         </table>
       </div>
      ...
      <div class="pseudoButton"  title="<?php echo i18n('changePassword');?>" onClick="requestPasswordChange();">
        <table style="width:100%">
          <tr>
            ...
            <td style="vertical-align:middle;"><?php echo pq_ucfirst(i18n('changePassword'));?>&nbsp;&nbsp;</td>
          </tr>
        </table>
      </div>
    </td>
...
<?php if ( ( ! isset($lockPassword) or $lockPassword==false) and ( ! isset($lockPasswordGuest) or $lockPasswordGuest==false or getSessionUser()->idProfile!=5) ) {?>
<tr style="height:40px">
