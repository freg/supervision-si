...
// MTY - MULTI CALENDAR      
   ...
   $resetPasswordEnabled = (Parameter::getGlobalParameter('passwordResetEnabled')=='YES')?true:false;
   setSessionValue('resetPasswordChangeInProgress', 'false');
?> 
<!DOCTYPE html>
<html>
<head>
...
<?php }?>  
  ...
  <script type="text/javascript" src="<?php echoStaticFileNameWithCacheMgt('../view/js/projeqtorDataTools.js');?>" ></script>
...
?>
  ...
  <div style="position:absolute;bottom:0;width:100%;height:16px;text-align:center;color:#eeeeee;text-shadow: 1px 1px 2px #555555;display:none">instance</div>
...
//           }
  ...
  <table align="center" width="100%" height="100%" class="<?php echo (isNewGui())?'':'loginBackground';?>">
    <tr height="100%">
	    <td width="100%" align="center">
	      <div class="login projeqtorReference background <?php  echo (isNewGui())?'loginFrameNewGui':'loginFrame' ;?>" style="position:relative;">
	          <button id="otpBackButtonRow" type="button" class="otpBackIcon" style="display:none;"
	          title="<?php echo i18n('buttonBackToLogin');?>" aria-label="<?php echo i18n('buttonBackToLogin');?>"
	          onClick="returnToStandardLogin();return false;">
	            <span class="otpBackArrow" aria-hidden="true">&#8617;</span>
	          </button>
			    ...
			    <tr style="height:100%" height="100%">
                  ...
                  <br/><br/>
			            <table width="100%">
			              ...
			              <tr id="otpLoginRow" style="display:none;">
			                <td colspan="3" title="<?php echo i18n('otpCode');?>">
			                  <div class="otpLoginPanel">
			                    <div class="otpVerificationTitle"><?php echo i18n('otpVerificationTitle');?></div>
			                    <div class="otpVerificationInstructions"><?php echo i18n('otpVerificationInstructions');?></div>
			                    <div id="otpCodeInputWrapper" class="otpCodeInputWrapper" onClick="selectOtpDigit(event);">
			                      <div id="otpDigitBoxes" class="otpDigitBoxes" aria-hidden="true">
			                        <?php for ($otpDigitIndex=0; $otpDigitIndex<6; $otpDigitIndex++) {?>
			                          <span class="otpDigitBox">&nbsp;</span>
			                        <?php }?>
			                      </div>
			                      <input tabindex="3" id="otpCode" name="otpCode" type="text" inputmode="numeric" pattern="[0-9]*"
			                      autocomplete="one-time-code" maxlength="6" class="otpCodeInput" aria-label="<?php echo i18n('otpCode');?>"
			                      onInput="updateOtpDigitBoxes();" onKeyUp="updateOtpDigitBoxes();" onSelect="updateOtpDigitBoxes();"
			                      onFocus="setOtpInputFocus(true);" onBlur="setOtpInputFocus(false);" />
			                    </div>
			                    <input type="hidden" id="otpAction" name="otpAction" value="" />
			                    <div id="otpResendRow" class="otpResendText">
			                      <span><?php echo i18n('otpCodeNotReceived');?></span>
			                      <button tabindex="5" id="otpResendButton" type="button" class="otpResendLink" onClick="requestNewOtp();return false;">
			                        <?php echo i18n('buttonResendOtp');?>
			                      </button>
			                    </div>
			                  </div>
			                </td>
			              ...
			              <tr id="rememberMeRow" style="height:30px">
			                <td></td>
			                <?php if(!isNewGui()){ ?>
			                ...
			                <?php }else{?>
			                   <td style="<?php if(isNewGui()) echo "font-size:12px;height:32px;padding-top: 10px;";?>">
			                     <div style="width:auto;text-align:center;float:left;" class="switchLogin">
                                  ...
                                  <?php if($showPassword and isNewGui() and !$resetPasswordEnabled){?>
                                  ...
                                  <?php }else if($resetPasswordEnabled){ ?>
                                  <div  id="resetPasswordButton" style="float:right;" class="largeTextButton passwordButtonNewGui" onClick="requestResetPasswordChange();return false;" > <?php echo i18n('buttonResetPassword') ?></div> 
                                  <?php }?>
                               </td>
			                <?php }?>
			                <td></td>
  ...
  <?php }?>
			              ...
			              <tr>
			                ...
			                <td colspan="3" style="position:fixed;width:100%; height:100%;z-index:99;">
			                  ...
			                  <div id="loginResultDiv" dojoType="dijit.layout.ContentPane" region="none" style="margin-left: 6px;">
			                    ...
			                    <?php if (Parameter::getGlobalParameter('applicationStatus')=='Closed'
			                          ...
			                          } else if (pq_array_key_exists('lostConnection',$_REQUEST)) {
