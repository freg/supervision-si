...
$year=0;
if (pq_array_key_exists('yearSpinnerT',$_REQUEST)) {
  $year=$_REQUEST['yearSpinnerT'];
}

$week=0;
if (pq_array_key_exists('weekSpinnerT',$_REQUEST)) {
  $week=$_REQUEST['weekSpinnerT'];
}
if (intval($week)<10) $week='0'.intval($week);

$month=pq_substr(date('Y-m-d',firstDayofWeek($week,$year)),5,-3);

$showPlannedWork=0;
if (pq_array_key_exists('showPlannedWork',$_REQUEST)) {
    $showPlannedWork=1;
}
...
$showId=0;
if (pq_array_key_exists('showIdT',$_REQUEST)) {
  $showId=1;
}
...
$displayProjectColorImputation=0;
if (pq_array_key_exists('displayProjectColorImputation',$_REQUEST)) {
  $displayProjectColorImputation=1;
}
Parameter::storeUserParameter('imputationDisplayProjectColorImputation', $displayProjectColorImputation);

$displayActivityColorImputation=0;
if (pq_array_key_exists('displayActivityColorImputation',$_REQUEST)) {
  $displayActivityColorImputation=1;
}
Parameter::storeUserParameter('imputationDisplayActivityColorImputation', $displayActivityColorImputation);
...
<?php 
ImputationLine::drawLines($userId, $rangeType, $rangeValue, $idle, $showPlannedWork, false, $hideDone, $hideNotHandled, $displayOnlyCurrentWeekMeetings, $week, $year, $showId,$hidePausedItem, $hideOffDays,$displayProjectColorImputation,$displayActivityColorImputation);
?>
</form>