...
if($hideEmailsSent=='')$hideEmailsSent='NO';
...
if(isset($isTodayScreen)){
  if($isTodayScreen){
    $critWhere.=" and (idProject is null or idProject in ".getVisibleProjectsList(false,'*').')';
    $where.=" and (idProject is null or idProject in ".getVisibleProjectsList(false,'*').')';
    $clause.=" and (idProject is null or idProject in ".getVisibleProjectsList(false,'*').')';
  }
}
if (Sql::isPgsql()) {
...
if ($countIdNote == 0 and $nbHistInfo==0 and $countMail==0) {
  echo "<div style='padding:10px;width:100%;text-align:center;font-style:italic;color:#A0A0A0'>".i18n ( "noDataToDisplay" )."</div>";
} else {
