...
if (! $comboDetail and pq_array_key_exists($filterObjectClass . "FilterLayout", $user->_arrayFilters)) {
  $idLayout=$user->_arrayFilters[$filterObjectClass . "FilterLayout"];
}
...
// 	}
	...
	foreach ($filterArray as $key => $filter) {
	  if ((!isset($filter['hidden']) or $filter['hidden']=='0') and !filterIsGroupOnly($filter)) {
	    $index=$key;
	    break;
	  }
	}
