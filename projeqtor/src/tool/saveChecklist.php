...
if (! pq_array_key_exists('checklistDefinitionId',$_REQUEST)) {
	throwError('checklistDefinitionId parameter not found in REQUEST');
}
$checklistDefinitionId=pq_trim($_REQUEST['checklistDefinitionId']); // validated to be numeric value in SqlElement base constructor
if (! pq_array_key_exists('checklistId',$_REQUEST)) {
	throwError('checklistId parameter not found in REQUEST');
}
$checklistId=pq_trim($_REQUEST['checklistId']); // validated to be numeric value in SqlElement base constructor
if (! pq_array_key_exists('checklistObjectClass',$_REQUEST)) {
...
if (! pq_array_key_exists('checklistObjectId',$_REQUEST)) {
	throwError('checklistObjectId parameter not found in REQUEST');
}
$checklistObjectId=pq_trim($_REQUEST['checklistObjectId']);
Security::checkValidId($checklistObjectId);

if (! pq_array_key_exists('checklistComment',$_REQUEST)) {
	throwError('checklistCommentd parameter not found in REQUEST');
}
$comment=pq_trim($_REQUEST["checklistComment"]);
...
$changed=false;
if ($checklist->comment!=pq_trim($comment)) {
  $changed=true;
}
