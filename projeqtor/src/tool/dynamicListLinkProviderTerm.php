...
$listFull=array();
$split=pq_explode('</option><option',$listOption);
foreach ($split as $string) {
	if (!pq_trim($string)) continue;
	$start=pq_strpos($string,'value="')+7;
  if (!$start) continue;
  $end=pq_strpos($string,'"',$start);
  $val=pq_substr($string,$start,$end-$start);
