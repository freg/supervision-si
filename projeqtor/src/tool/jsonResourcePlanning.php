...
$selectResource=null;
if (pq_array_key_exists('selectResourceName',$_REQUEST)) {
...
$selectTeam=null;
if (pq_array_key_exists('teamName',$_REQUEST)) {
...
if (pq_array_key_exists('organizationName',$_REQUEST)) {
...
and ( ! pq_array_key_exists('idProject',$_REQUEST) or pq_trim($_REQUEST['idProject'])=="")) {
...
// if ( pq_array_key_exists('showProject',$_REQUEST) ) {
//   $showProject=true;
// }
$showProject = RequestHandler::getValue('showProject');
...
// if ( pq_array_key_exists('listShowNullAssignment',$_REQUEST) ) {
//   $showNullAssignment=true;
// }
$hideAssignationWihtoutLeftWork=(int)Parameter::getUserParameter("hideAssignationWihtoutLeftWork");
$showNullAssignment=!$hideAssignationWihtoutLeftWork;
if (! $showNullAssignment) {
   $queryWhere.= ($queryWhere=='')?'':' and ';
   $queryWhere.=" (ass.leftWork>0)";
}

$listShowIdle=(int)Parameter::getUserParameter("planningShowClosed");
if (! $listShowIdle) {
  $queryWhere.= ($queryWhere=='')?'':' and ';
  $queryWhere.=" (ass.idle=0)";
}
...
if(!$showProjectModel or $showProjectModel=='false'){
  $queryWhere.= ($queryWhere=='')?'':' and ';
  $queryWhere.=  $table . ".idProject not in " . Project::getTemplateInClauseList() ;
}
$ass=new Assignment();
$res=new Resource();
$querySelect .= "pe.idProject as idProj, pe.id idPe, pe.wbs wbs, pe.wbsSortable wbsSortable, pe.priority priority, pe.idplanningmode idplanningmode, pe.validatedstartdate, pe.validatedenddate, pe.validatedduration, pe.notplannedwork, pe.plannedenddate as peplannedend, pe.plannedstartdate as peplannedstart, pe.color as color, pe.hatchPattern as hatchpattern, ass.*, usr.fullName as name, pe.refName refName";
$querySelect .= ", pe.topRefType as topreftype, pe.toprefid as toprefid, pe.topid as topid, pe.paused ";
...
$queryWhere.= " AND NOT pe.refType='PeriodicMeeting' ";
  ...
  $cptLine=0; 
	while ($line = Sql::fetchLine($result)) {
		...
		if ($line['idresource']!=$idResource) {
			...
			$resAr["idresource"]=null;
			$resAr["progress"]=0;
			$resAr["topid"]=0;
			$resAr["leftwork"]=0;
			$resAr["priority"]="";
			$resAr["planningmode"]="";
			setResourcePlanningTags($resAr, $columnsDescription);
      ...
      $resPr["idresource"]=$prj->idResource;
      $resPr["progress"]=0;
      $resPr["topid"]=$idRes;
      $resPr["leftwork"]=0;
      $resPr["priority"]="";
      $resPr["planningmode"]="";
      setResourcePlanningTags($resPr, $columnsDescription);
    ...
    if (!isset($line["id"])) $line["id"]=$line["idpe"];
      ...
      $line["type"]=(property_exists($item,$type))?SqlList::getNameFromId('Type',$item->$type):null;
		...
		setResourcePlanningTags($line, $columnsDescription);
		$list[]=$line;
		//$sumValidated=0;
    ...
    $sumLeft+=$line["leftwork"];
      ...
      if ($line['plannedstartdate'] ) {
		...
		$list[$keyRes]["validateddurationdisplay"]='';
	    ...
	    $list[$keyProj]["validateddurationdisplay"]='';
	...
	if ($cptLine==0) {
	  if ($print) {
	    ...
	    echo '{"identifier":"id", "items":[ ],"totalRows":"0"}';
	  }
	  return;
	}
	if ($print) {
    ...
    echo ' "items":[';
        ...
        echo '"' . htmlEncode($id) . '":"' . $val . '"';
			...
			//add expanded status
			if (($line['reftype']=='Resource' or $line['reftype']=='ResourceTeam' or $line['reftype']=='Project') and pq_array_key_exists('Planning_'.$line['reftype'].'_'.$line['refid'], $collapsedList)) {
				echo ',"collapsed":"1"';
			} else {
				echo ',"collapsed":"0"';
			}
			...
			foreach ($depList as $dep) {
				if ( isset($arrayPeAss[$dep->predecessorId])) {
					foreach($arrayPeAss[$dep->predecessorId] as $assId) {
						...
						if (isset($arrayResource[$idResource]) and pq_array_key_exists($assId,$arrayResource[$idResource])) {
							$listPred.=($listPred!="")?',':'';
							$listPred.=$assId;
						}
					}
			...
			if ($line['reftype']!='Fixed' and $line['reftype']!='Construction' and $line['reftype']!='Replan') {
			  if (0 and isset($line['idresource']) and $line['idresource'] ) {
            ...
            //typeColorActivity
            if(($ParamDefaultTypeColor=="YES") and $line['reftype']=='Activity') {
              if(isset($line["color"])) {
                $lineColor=((isset($line['color']))?pq_trim($line['color'],'#'):'');
              }else{ 	
			  $item=new Activity($line['refid'],true); // TODO Remove
            ...
            //colorActivity
            if((Parameter::getUserParameter('showColorActivity')=="1")) {
              $lineColor=((isset($line['color']))?pq_trim($line['color'],'#'):'');
            }  	
		//noCheck	
...
function displayGantt($list) {
  ...
  $showWork=false;
  if ( pq_array_key_exists('showWork',$_REQUEST) ) {
    $showWork=true;
  }
	// calculations
    ...
    foreach ($sortArray as $col) {
		...
		foreach ($resultArray as $line) {
			...
			if ($pGroup) {
        $collapsed=false;
        if (pq_array_key_exists($scope, $collapsedList)) {
          $collapsed=true;
...
function getPlanningHatchPattern($line) {
  ...
  if (!empty($line['reftype']) && $line['reftype'] == 'Activity' && !empty($line['refid'])) {
...
function setResourcePlanningTags(&$line, $columnsDescription) {
  $line['tags']='';
  $line['tagsdisplay']='';
  if (!isset($columnsDescription['Tags']) or $columnsDescription['Tags']['show']!=1) return;
  if (!isset($line['reftype']) or !isset($line['refid'])) return;
  if (property_exists($line['reftype'], 'tags')) {
    $line['tags']=SqlList::getFieldFromId($line['reftype'], $line['refid'], 'tags');
    $line['tagsdisplay']=tagFormatter($line['tags']);
  }
}
?>
