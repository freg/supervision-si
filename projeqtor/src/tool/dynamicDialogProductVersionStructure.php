...
$structureId=null;
if (pq_array_key_exists('structureId',$_REQUEST)) {
...
if($objectClass=='Asset'){
  $way='structure';
  $str=new ProductAsset($structureId);
} else {
  if (pq_array_key_exists('way',$_REQUEST)) {
    $way=$_REQUEST['way'];
  }
...
if ($way=='composition' and $directAccessToList=='true') {
  ...
  if ($cvListId=='(') {
    ...
    $cvListId=pq_substr($cvListId, 0, -2).')';
    $cvListName=pq_substr($cvListName, 0, -2);
  }
  ...
  <tr>
    <td>
      <form id='productVersionStructureForm' name='productVersionStructureForm' onSubmit="return false;">
        ...
        <table>
          <tr><td>&nbsp;</td><td>&nbsp;</td></tr>
          <tr><td colspan="2" class="section"><?php echo i18n('sectionVersion'.pq_ucfirst($way),array(i18n($objectClass),intval($objectId).' '.$object->name));?></td></tr>  
          <tr><td>&nbsp;</td><td>&nbsp;</td></tr>
        <?php if (0) {?>
          <tr>
                ...
                <?php if (!$structureId) echo 'multiple';?> class="selectList" onchange="enableWidget('dialogProductVersionStructureSubmit');"  ondblclick="<?php if($objectClass=='Asset'){?>saveProductVersionStructureAsset();<?php }else{?>saveProductVersionStructure();<?php }?>" value="">
                  <?php 
                    if (!$structureId) { 
                      ...
                      if($objectClass =='Asset'){
                        htmlDrawOptionForReference('idVersion', $str->idProductVersion, null, true, 'id', $str->idProductVersion);
                      } else {
