...
if (! pq_array_key_exists('otherVersionRefType',$_REQUEST)) {
...
foreach ($arrayId as $versId) {
	...
	if (! $obj->$objVersionFld) {
		...
		if ((! $otherVersion or ! $otherVersion->id) and $versId!=$obj->$objVersionFld) {
		  ...
		  if (!$result) {
		    $result=$res;
		  } else if (pq_stripos($res,'id="lastOperationStatus" value="OK"')>0 and !$updatedMain) {
