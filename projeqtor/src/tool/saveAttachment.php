...
$isIE=false;
if(RequestHandler::getValue('SendMail')){
  $sendMail = RequestHandler::getValue('SendMail');
}else{
  $sendMail="false";
}
if(RequestHandler::getValue('attachmentMode')){
  $attachmentMode = RequestHandler::getValue('attachmentMode');
}else{
...
$type='file';
if (! pq_array_key_exists('attachmentType',$_REQUEST)) {
...
if(!RequestHandler::isCodeSet("attachmentRefId") or RequestHandler::getValue("attachmentRefId")==''){
  $error=htmlGetWarningMessage(i18n("saveSelectItemBeforAttaching"));
...
if ($type=='file') {
  if (pq_array_key_exists('attachmentFile',$_FILES)) {
    $uploadedFileArray[]=$_FILES['attachmentFile'];
  } else if (pq_array_key_exists('uploadedfile0',$_FILES)) {
  	$cnt = 0;
  	while(isset($_FILES['uploadedfile'.$cnt])){
  		$uploadedFileArray[]=$_FILES['uploadedfile'.$cnt];
  		$cnt++;
  	}
  } else if (pq_array_key_exists('attachmentFiles',$_FILES) and pq_array_key_exists('name',$_FILES['attachmentFiles'])) {
  ...
  } else if ($nameDiv and pq_array_key_exists($nameDiv,$_FILES) and pq_array_key_exists('name',$_FILES[$nameDiv])){
  ...
  if (! pq_array_key_exists('attachmentLink',$_REQUEST)) {
    ...
    $link=Security::checkValidUrl($link,false);
	  if($link===false) {
...
if (! $error) {
  if (! pq_array_key_exists('attachmentRefType',$_REQUEST)) {
  	if (!$obj) {
...
if (! $error) {
  if (pq_array_key_exists('attachmentRefId',$_REQUEST)) { // Retrieve from request
...
if (! $error) {    
  if (! pq_array_key_exists('attachmentDescription',$_REQUEST)) {
    $attachmentDescription="";
  } else {
    $attachmentDescription=$_REQUEST['attachmentDescription'];
  }
}
if (! pq_array_key_exists('attachmentPrivacy',$_REQUEST)) {
  $idPrivacy=1;
} else  {
...
foreach ($uploadedFileArray as $uploadedFile) {
  $attachment=new Attachment();
	if (! $error) {
	  if($refType=='SubTask' and pq_trim(pq_strpos($uploadedFile['type'], 'image'))=='' and pq_trim(pq_strpos($uploadedFile['type'], 'pdf'))==''){
	  ...
	  if ($type=='file') {
	    if ($attachmentMaxSize!=='' and intval($uploadedFile['size']) > intval($attachmentMaxSize)) {
	      $error.=htmlGetErrorMessage(i18n('errorTooBigFile',array($attachmentMaxSize,'paramAttachmentMaxSize')));
	      errorLog(i18n('errorTooBigFile',array($attachmentMaxSize,'paramAttachmentMaxSize')));
	      break;
	    }
	    $attachment->fileName=pq_trim($uploadedFile['name']);
...
// 	    }
//    Previous replace now done in Security::checkValidFileName(xx,xx,true)
      $attachment->fileName=htmlEncode($attachment->fileName,'attachment');
	    $attachment->fileName=Security::checkValidFileName($attachment->fileName,true, true);
	  ...
	  if(($refType=="Resource" || $refType=="Project") && $attachmentMode=="profilePic"){
	    $attachment->isProfilePic=1;
	  }else{
	    $attachment->isProfilePic=0;
	  }
	  if($sendMail=="false"){
	    $subResult=$attachment->save();
	  }else{
	    $subResult='inserted <input type="hidden" id="lastOperation" value="insert" /><input type="hidden" id="lastOperationStatus" value="OK" />';
	  }
	  $newId=$attachment->id;
	  if (! $result) {
	  	$result=$subResult;
	  } else {
	  	$pos=pq_strpos($result, '#');
	  	if ($pos) {
	  	  $result=pq_substr_replace($result, '#'.$newId.', #', $pos,1);
	  	} 
    }  
	} 
	...
	if (! $error and $type=='file') {
	    ...
	    $uploaddir = $attachmentDirectory . $pathSeparator . "attachment_" . $attachment->refType ."_".$attachment->refId. $pathSeparator;
	    if (! file_exists($uploaddir)) {
	      mkdir($uploaddir,0777,true);
	    }
	  }
	...
	if (! $error and $attachment->idPrivacy==1) { // send mail if new attachment is public
	  $elt=new $refType($refId);
		...
		if ($refType=='Resource' && $attachment->isProfilePic==1) { // Also Includes User and Contact thanks to line ~156 
      Affectable::generateThumbs($refType, intval($refId), $attachment->subDirectory.$attachment->fileName);
    }
    if ($refType=='Project' && $attachment->isProfilePic==1) { 
//Todo
    }
...
$jsonReturn='{"file":"'.htmlEncodeJson($attachment->fileName).'",'
 ...
 .'"type":"'.$type.'",'
 .'"size":"'.htmlEncodeJson($attachment->fileSize).'"  ,'
 .'"message":"'.pq_str_replace('"',"'",$message).'"}';
