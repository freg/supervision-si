...
$mandatoryNoteOnStatusChange=RequestHandler::getBoolean('mandatoryNoteOnStatusChange');

$noteId=null;
if (pq_array_key_exists('noteId',$_REQUEST)) {
...
<div >
  <table style="width:100%;">
    <tr><td>
    <?php if ($mandatoryNoteOnStatusChange) { ?>
    <div class="messageWARNING" style="margin:0 0 8px 0;text-align: center;"><?php echo i18n('messageMandatoryNoteOnStatusChange');?></div>
    <?php } ?>
    <div id="noteInfoDiv" class="noteInfoDiv" style="display:none;"><span style="font-weight: bold;"><?php echo i18n('dialogNoteIn'); ?></span> <?php echo ' ' . $objectClass . ' #' . ltrim($objectId, '0'); ?></div>
    ...
    <tr>
      <td>
       <form id='noteForm' name='noteForm' onSubmit="return false;" >
         ...
         <input id="mandatoryNoteOnStatusChange" name="mandatoryNoteOnStatusChange" type="hidden" value="<?php echo ($mandatoryNoteOnStatusChange)?'1':'0';?>" />
         ...
         <?php if (getEditorType()=="CK" or getEditorType()=="CKInline") {?> 
          ...
          if (!isTextFieldHtmlFormatted($note->note)) {
          	...
          	echo pq_htmlspecialchars($note->note);
          } ?></textarea>
        ...
        <?php } else {?>
          <textarea dojoType="dijit.form.Textarea" type="hidden"
           id="noteNote" name="noteNote"
           style="display:none;"><?php echo pq_htmlspecialchars($note->note);?></textarea>    
    ...
    <tr>
      <td align="center">
        ...
        <button class="mediumTextButton"  dojoType="dijit.form.Button" type="button" 
          onclick="dijit.byId('dialogNote').hide(); var minimizedIcon=dojo.query('[id^=noteEditorMinimizedIcon_]').shift(); if (minimizedIcon) minimizedIcon.parentNode.removeChild(minimizedIcon);  repositionMinimizedIcons();">
