...
foreach($lst as $alert) {
	if ($alert->alertDateTime<=$date) {
	  if ($alert->refType and $alert->refId) {
	    echo '<span class="alertSpanGoto" style="cursor:pointer;" onClick="closeAlertBox();gotoElement(\''.htmlEncode($alert->refType, 'protectQuotes').'\',\''.htmlEncode($alert->refId, 'protectQuotes').'\');"><b>' . htmlEncode($alert->refType) .' #'.htmlEncode($alert->refId, 'protectQuotes'). '</b></span>';
	  } else {
	    echo '<b>' . $alert->title . '</b>';
	  }
	  echo '<br/>';
	  echo  $alert->message;
