...
if ($filterObjectClass!='Planning' and $filterObjectClass!='GlobalPlanning' and $filterObjectClass!='VersionsPlanning' and $filterObjectClass!='ResourcePlanning' and $filterObjectClass!='PlanningWorkPlan' and $filterObjectClass!='ProjectDashboard') Security::checkValidClass($filterObjectClass, 'filterObjectClass');
...
$flt=new Filter();
$critFilterObjectClass=($filterObjectClass=='Planning' or $filterObjectClass=='PlanningWorkPlan')?'PlanningElement':($filterObjectClass=='ProjectDashboard' ? 'Project' :$filterObjectClass );
$orderByFilter = "sortOrder ASC";
$crit=array('idUser'=> $user->id, 'refType'=>$critFilterObjectClass);
$filterList=$flt->getSqlElementsFromCriteria($crit,false,null,$orderByFilter);
$comboDetail=false;
if (RequestHandler::isCodeSet('comboDetail')) {
  $comboDetail=true;
}
htmlDisplayStoredFilter($filterList,$filterObjectClass,"","",$comboDetail);
