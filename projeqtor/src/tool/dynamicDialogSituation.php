...
$situationId=null;
if (pq_array_key_exists('situationId',$_REQUEST)) {
...
if($situation->date){
  $dateTime = pq_explode(' ', $situation->date);
  $date = $dateTime[0];
  $time = $dateTime[1];
}else{
  $date = date('Y-m-d');
        ...
        <td colspan="2">
          ...
          <div style="width:800px;">
            <textarea style="width:800px; height:<?php echo $detailHeight;?>px"
              ...
              if (!isTextFieldHtmlFormatted($situation->comment)) {
              	...
              	echo pq_htmlspecialchars($situation->comment);
              } ?></textarea>
          </div>
          ...
          <?php } else {?>
            <textarea dojoType="dijit.form.Textarea" type="hidden"
             id="situationComment" name="situationComment"
             style="display:none;"><?php echo pq_htmlspecialchars($situation->comment);?>
            </textarea>    
