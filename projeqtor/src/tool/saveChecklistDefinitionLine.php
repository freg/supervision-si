...
$lineId=null;
if (pq_array_key_exists('checklistDefinitionLineId',$_REQUEST)) {
...
$lineName=null;
if (pq_array_key_exists('dialogChecklistDefinitionLineName',$_REQUEST)) {
	$lineName=$_REQUEST['dialogChecklistDefinitionLineName'];
}
$lineTitle=null;
if (pq_array_key_exists('dialogChecklistDefinitionLineTitle',$_REQUEST)) {
	$lineTitle=$_REQUEST['dialogChecklistDefinitionLineTitle'];
}
$sortOrder=0;
if (pq_array_key_exists('dialogChecklistDefinitionLineSortOrder',$_REQUEST)) {
...
$checkTitles=array();
for ($i=1;$i<=5;$i++) {
	if (pq_array_key_exists('dialogChecklistDefinitionLineChoice_'.$i,$_REQUEST)) {
		$checkName=$_REQUEST['dialogChecklistDefinitionLineChoice_'.$i];
		if (pq_trim($checkName)) {
			$checkNames[]=$checkName;
			if (pq_array_key_exists('dialogChecklistDefinitionLineTitle_'.$i,$_REQUEST)) {		
...
$exclusive=0;
if (pq_array_key_exists('dialogChecklistDefinitionLineExclusive',$_REQUEST)) {
	$exclusive=1;
}
