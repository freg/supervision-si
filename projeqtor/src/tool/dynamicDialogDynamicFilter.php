...
if (! pq_array_key_exists('idFilter',$_REQUEST)) {
        ...
        <form id='dialogDynamicFilterForm' name='dialogDynamicFilterForm' onSubmit="return false;">
         ...
         <table width="100%" style="border: 1px solid grey;">
           ...
           foreach ($filter->_FilterCriteriaArray as $filterCriteria) {
           	...
           	// PBER : fix incorrect foreign key name
           	if (pq_substr($filterCriteria->dispAttribute, 0,1)=='[') {
           	  $sqlAttributeField=planningGetFilterAttributeField($filterCriteria->sqlAttribute);
           	  if (isForeignKey($sqlAttributeField, $objectClass)) {
           	    $obj=new $objectClass();
           	    $filterCriteria->dispAttribute=$obj->getColCaption($sqlAttributeField);
           	    $filterCriteria->save();
           	  }
           	}
           	//gautier 
            $today = i18n('today');
           	if(pq_trim($filterCriteria->dispOperator) == "<= ".$today or pq_trim($filterCriteria->dispOperator)== ">= ".$today){
                 ...
                 if($filterCriteria->sqlAttribute=='idBusinessFeature'){
                   ...
                   foreach ($lstbf as $bf) {
                     ...
                     echo '<option value ="'.$bf->id.'">'.$bf->name.' ('.$product->name.')</option>';
                 	 ...
                 	 $listField=planningGetFilterAttributeField($filterCriteria->sqlAttribute);
                 	 if ($listField=='idTargetProductVersion' or $listField=='idOriginalProductVersion') $listField='idProductVersion';
                 	 $showIdle=true;
                 	 $critValue=null;
                 	 $critField=null;

                 	 if ($listField=='idResource') {
                 	   $showIdle=false;
                 	   $listField = 'idResourceAll'; 
                 	 }
                 	 $archiveOn = (sessionValueExists('projectSelectorShowIdle') and getSessionValue('projectSelectorShowIdle')==1)?1:0;
//                  	 if (($listField == 'idComponentVersion' || $listField == 'idVersion' || $listField == 'idTargetComponentVersion' || $listField == 'idOriginalComponentVersion' || $listField == 'idProductVersion') && $archiveOn!=1 ) {
//                        $showIdle=false;
//                    }
                   if ($archiveOn!=1) $showIdle=false;
                   htmlDrawOptionForReference($listField, null, null, true,null,null,false,null,$showIdle);
                 }
                 ?>
               </select> 
                 	<?php } else if (in_array(pq_trim($filterCriteria->sqlOperator), array('exists', 'not exists','LIKE', 'NOT LIKE','ILIKE','NOT ILIKE')) or $filterCriteria->sqlValue == 'int') { ?>
                    ...
                    <button style="display:block;margin-left:<?php echo (isNewGui())?'0':'-1';?>px; padding-right:2px;position:relative;top:-2px;" id="idButtonCombo<?php echo $cpt;?>" dojoType="dijit.form.Button" showlabel="false"
                      ...
                      <script type="dojo/connect" event="onClick" args="evt">
                         ...
                         var fieldTarget = 'filterValueList'+nb;
                          <?php
                          $fk=foreignKeyWithoutAlias(planningGetFilterAttributeField($filterCriteria->sqlAttribute));
                          $objectClassSelect=pq_substr($fk,2);
