...
//        $idStatus = SqlElement::getSingleSqlElementFromCriteria("Status", array("name" => "unread"))->id;
          ...
          while ($line) {
            if (Sql::isPgsql()) { // Must replace lowercase data with cased values
              foreach (array('idObjectNotif','typeId', 'objectClassName', 'notifiableItem') as $fld) {
                $line[$fld]=$line[pq_strtolower($fld)];
              }
            }
            $notifList[]=$line;
            $line = Sql::fetchLine($result);
        ...
        if (count($notifList)===0) {
            ...
            $iconRoot="iconSum";
            $row  = '{id:"' . $theId . '"';
            $row .= ', name:"'. pq_str_replace('"', "''", pq_ucfirst(i18n("colCountTotal"))) . ' ('.$totalCount.')"';
            $row .= ', objClass:""';
            $row .= ', objId:""';
...
// MTY - LEAVE SYSTEM
        ...
        foreach ($arrayTree as $type) {
            ...
            $nbRows++;
            $theId++;
            $iconRoot='icon' .pq_ucfirst(pq_strtolower($type["type"]));
            $row  = '{id:"'. $theId . '"';
            $row .= ', name:"'. pq_str_replace('"', "''",i18n($type["type"])) . ' ('.$type['count'].')"';
            $row .= ', objClass:""';
            $row .= ', objId:""';
            //$row .= ', iconClass:"icon' .pq_ucfirst(pq_strtolower($type["type"])).$paramIconSize. '"';
            $row .= ', iconClass:"'.$iconRoot.$paramIconSize.' '.$iconRoot.' iconSize'.$paramIconSize.'"';
            ...
            foreach($listObjClass as $objectClass) {
                ...
                $row  = '{id:"' . $theId . '"';
                $row .= ', name:"'. pq_str_replace('"', "''",$objectClass["objectClassName"]) . ' ('.$objectClass['count'].')"';
                $row .= ', objClass:""';
                $row .= ', objId:""';
               ...
               foreach($listDef as $definition) {
                    ...
                    $theId++;
                    $row  = '{id:"' . $theId . '"';
                    $row .= ', name:"'. pq_str_replace('"', "''",$definition["definition"]) . ' ('.$definition['count'].')"';
                    $row .= ', objClass:""';
                    $row .= ', objId:""';
                    ...
                    foreach($listIdObj as $objectId) {
                        ...
                        $theId++;
                        $row  = '{id:"' . $theId . '"';
                        $row .= ', name:"#'. pq_str_replace('"', "''",$objectId["idObject"]) . ' ('.$objectId['count'].')"';
