...
$imgName=RequestHandler::getValue('imageName');
$sizeAdd=RequestHandler::getValue('size');
...
if ($class=='Attachment') {
    $path = pq_str_replace('${attachmentDirectory}', Parameter::getGlobalParameter('paramAttachmentDirectory'), $obj->subDirectory);
    $name = $obj->fileName;
  ...
  $folderPath = $attachmentDirectory . $pathSeparator . "attachment_" . $class ."_".$id.$pathSeparator;
  $size = $sizeAdd;
  $name = $imgName;
...
$file = $file ?? '';
$ext = pq_strtolower ( pathinfo ( $file, PATHINFO_EXTENSION ) );
if ($ext=='log') {
  $type='text/plain';
}
if(!isset($type)){
...
//if (pq_array_key_exists('display',$_REQUEST)) {
//  $contentType=$type;
//}
if (pq_substr($name, -10)=='.projeqtor') {
	$name=pq_substr($name,0,pq_strlen($name)-10);
}
if (pq_substr($name, -14)=='.projeqtor.txt') {
  $name=pq_substr($name,0,pq_strlen($name)-14);
}

$name=pq_str_replace(array("\n","\r"),array('',''),$name);
if (($file != "") && (file_exists($file))) { 
  ...
  if (!pq_array_key_exists('showHtml', $_REQUEST)) {
