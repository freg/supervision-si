 ...
 */
  ...
  scriptLog('   ->/view/dynamicDialogShowUserStories.php');
  ...
  if ($refType=='Sprint') {
    $t=new UserStory();
    $list=$t->getSqlElementsFromCriteria(array('idSprint'=>$refId));
  }
  ...
  echo '</tr>';
  foreach ( $list as $userStory ) {
    $userId=$userStory->idUser;
    $class=get_class($userStory);
    $userName=SqlList::getNameFromId('User', $userId);
    $creationDate=$userStory->creationDate;
    $canGoto=(securityCheckDisplayMenu(null, $class) and securityGetAccessRightYesNo('menu'.$class, 'read', $userStory) == "YES")?true:false;
    echo '<tr>';
    $className=i18n($class);
    echo '<td class="linkData" style="white-space:nowrap;width:15%"><table><tr><td>'.formatIcon($class,16).'</td><td>&nbsp;'.$className .' #' . $userStory->id.'</td></tr></table>';
    echo '</td>';
    $goto="";
    if ($canGoto) {
      $goto=' onClick="dijit.byId(\'dialogShowUserStories\').hide();gotoElement(' . "'" . $class . "','" . htmlEncode($userStory->id) . "'" . ');" style="cursor: pointer;" ';
    }
    echo '<td class="linkData '.((isNewGui() and isset($goto) and $goto!='')?'classLinkName':'').'" ' . $goto . ' style="position:relative;width:35%">';
    echo htmlEncode($userStory->name);
    echo formatUserThumb($userId, $userName, 'Creator');
    echo formatDateThumb($userStory->creationDate, null);
    echo '</td>';
    $workElement = $userStory->WorkElement;
