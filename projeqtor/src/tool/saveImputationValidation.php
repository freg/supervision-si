...
if($buttonAction == 'validateSelection'){
  $idWorkPeriod = RequestHandler::getValue('idWorkPeriod');
  $arrayId = pq_explode(',', $idWorkPeriod);
  if (count($arrayId)>100) traceLog("Validation in Progress for ".count($arrayId)." items");
  $cpt=0;
  $cptDone=0;
  foreach ($arrayId as $id){
    ...
    if ($cpt%100==0) {
      if (count($arrayId)>100) traceLog("   => $cpt done");
      projeqtor_set_time_limit(500);
    }
  }
  if (count($arrayId)>100 and $cpt%100!=0) traceLog("   => $cpt done");
}
// commit workPeriod
Sql::commitTransaction();
