...
$timeForSpeaker=0;
if ( pq_array_key_exists ( 'timeForSpeaker', $_REQUEST )) {
  $timeForSpeaker=pq_explode('|',$_REQUEST['timeForSpeaker']);
}
$idSpeaker=-1;
if ( pq_array_key_exists ( 'idSpeaker', $_REQUEST )) {
  $idSpeaker=pq_explode('|',$_REQUEST['idSpeaker']);
}
$color='';
if ( pq_array_key_exists ( 'color', $_REQUEST )) {
  $color=pq_explode('|',$_REQUEST['color']);
}
$organizator='';
if ( pq_array_key_exists ( 'allOrganizator', $_REQUEST )) {
  $organizator=pq_explode('|',$_REQUEST['allOrganizator']);
}

$noHistory=true;
if ( pq_array_key_exists ( 'noHistory', $_REQUEST )) {
  $noHistory=$_REQUEST['noHistory']==1;
}

$canSpeak='';
if ( pq_array_key_exists ( 'allCanSpeak', $_REQUEST )) {
  $canSpeak=pq_explode('|',$_REQUEST['allCanSpeak']);
}
if (! pq_array_key_exists ( 'idLiveMeeting', $_REQUEST )) {
...
if(count($meeting->_Assignment)!=0){
  ...
  if(pq_array_key_exists ( 'liveMeetingTimeOrganizator', $_REQUEST ) && pq_array_key_exists ( 'liveMeetingTime', $_REQUEST )){
    $json=json_decode($liveMeeting->param,true);
