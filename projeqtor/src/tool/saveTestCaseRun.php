...
$testCaseList = null;
if ($mode=='add') {
	if (! pq_array_key_exists('testCaseRunTestCaseList',$_REQUEST)) {
...
$testCase = null;
if ($mode=='edit'){
  if (! pq_array_key_exists('testCaseRunTestCase',$_REQUEST)) {
...
if (! pq_array_key_exists('testCaseRunComment',$_REQUEST)) {
  throwError('testCaseRunComment parameter not found in REQUEST');
}
$comment=($_REQUEST['testCaseRunComment']);

$status=1;
if ( pq_array_key_exists('testCaseRunStatus',$_REQUEST)) {
  $status=($_REQUEST['testCaseRunStatus']);
}

$ticket=null;
if (pq_array_key_exists('testCaseRunTicket',$_REQUEST)) {
...
if ($mode=='add'){
  if (pq_array_key_exists('testCaseRunAllowDuplicate',$_REQUEST)) {
    $allowDuplicate=true;
  }
}

$sortOrder=0;
if (pq_array_key_exists('dialogTestCaseRunSortOrder',$_REQUEST)) {
...
$session=new TestSession($session);
$session->updateDependencies();
if ($cpt>10) {
  $deb=pq_strpos($result,'#');
  $last=pq_stripos($result,'#');
  $fin=pq_stripos($result,' ',$last);
	$result=pq_substr($result, 0, $deb).' : '.$cpt.pq_substr($result,$fin);
} 
