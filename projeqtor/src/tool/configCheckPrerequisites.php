...
function checkPrerequisites($showOK=false,$dbType=null) {
  ...
  if ( ! extension_loaded('pdo_'.$dbType) ) {
    showError("Module PDO for '" . pq_strtoupper($dbType)."' is not available - check your php configuration (php.ini)");
    $checkErrors++;
  } else {
    if ($showOK) showMessage("Module PDO for '" . pq_strtoupper($dbType)."' is available : OK");
    $checkOK++;
  }
...
//   }
  ...
  $ini_val=pq_strtolower(ini_get('file_uploads'));
