...
foreach ($listColumns as $col) {
	if ( ! SqlElement::isVisibleField($col->attribute) or ($objectClass=='GlobalView' and $col->field=='id') ) {
	  ...
	  if (isNewGui()) {
		echo '<div style="width:100%;height:34px;cursor:default" class="dojoDndItem" id="listColumnSelectorId'.htmlEncode($col->id).'" data-field-id="'.$cpt.'" dndType="planningColumn">';
  		if ($col->attribute=='id') {
	    ...
	    echo '<div style="width:100%;" class="dojoDndItem" id="listColumnSelectorId'.htmlEncode($col->id).'" data-field-id="'.$cpt.'" dndType="planningColumn">';
	    if ($col->attribute=='id') {
