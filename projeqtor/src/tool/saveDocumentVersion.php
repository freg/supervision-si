...
if (pq_array_key_exists('documentVersionId',$_REQUEST)) {
...
if (! $documentVersionId) { // Get file only on insert
	if (pq_array_key_exists('documentVersionLink',$_REQUEST)) {
		$documentVersionLink=$_REQUEST['documentVersionLink'];
	}
	if (pq_array_key_exists('documentVersionFile',$_FILES)) {
	...
	if (Parameter::getGlobalParameter('versionsWithoutAttachments')!='YES'){
...
if (!$error) {
	if (! pq_array_key_exists('documentVersionNewVersion',$_REQUEST)) {
...
if (!$error) {
	if (! pq_array_key_exists('documentVersionNewRevision',$_REQUEST)) {
...
if (!$error) {
	if (! pq_array_key_exists('documentVersionNewDraft',$_REQUEST)) {
...
$documentId=null;
if (!$error) {
	if (! pq_array_key_exists('documentId',$_REQUEST)) {
...
if (!$error) {
	if (! pq_array_key_exists('documentVersionDate',$_REQUEST)) {
...
if (pq_array_key_exists('documentVersionIsRef',$_REQUEST)) {
  $documentVersionIsRef=1;
}

$documentVersionIdStatus=null;
if (!$error) {
	if (! pq_array_key_exists('documentVersionIdStatus',$_REQUEST)) {
...
if (!$error) {
	if (! pq_array_key_exists('documentVersionDescription',$_REQUEST)) {
...
if (!$error) {
	if (! pq_array_key_exists('documentVersionNewVersionDisplay',$_REQUEST)) {
...
if (! $error) {
  ...
  if (! $documentVersionId) {
	  if ($documentVersionLink) {
	    ...
	    if (Parameter::getGlobalParameter('versionsWithoutAttachments')!='YES' or $uploadedFile){
	      $dv->fileName=$uploadedFile['name'];
	      $dv->mimeType=$uploadedFile['type'];
	      $dv->fileSize=$uploadedFile['size'];
	    }else{
	      $dv->fileName='noFileNameAdmitted';
	    }
	  }
  }
...
if (! $documentVersionId and ! $error and !$invalidControl) {
	if (! $error and !$documentVersionLink ) {
		...
		$split=pq_explode($pathSeparator,$uploadfile);
		...
		if (strpos($uploadfile, 'noFileNameAdmitted') == false or Parameter::getGlobalParameter('versionsWithoutAttachments')!='YES'){
