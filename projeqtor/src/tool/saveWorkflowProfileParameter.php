...
if (! pq_array_key_exists('workflowId',$_REQUEST)) {
	throwError('workflowId parameter not found in REQUEST');
}
$workflowId=pq_trim($_REQUEST['workflowId']);
...
foreach($profileList as $idProfile=>$profile) {
  $critArray=array('idWorkflow'=>$workflowId, 'idProfile'=>$idProfile);
	...
	if (pq_array_key_exists('dialogWorkflowParameterCheckProfileId_'.$idProfile,$_REQUEST) ) {
		$cs->checked=1;
	} else {
		$cs->checked=0;
	}
	$resultLine=$cs->save();
	if (! $result or pq_stripos($resultLine,'id="lastOperationStatus" value="ERROR"')>0) {
	 	$result=$resultLine;
	}
}
