...
$idAssignment=-1;
if (pq_array_key_exists('idAssignment',$_REQUEST)) {
  $idAssignment=$_REQUEST['idAssignment'];
}

$week=-1;
if (pq_array_key_exists('week',$_REQUEST)) {
  $week=$_REQUEST['week'];
}
...
$refType=-1;
if (pq_array_key_exists('refTypeComment',$_REQUEST)) {
  $refType=$_REQUEST['refTypeComment'];
}

$refId=-1;
if (pq_array_key_exists('refIdComment',$_REQUEST)) {
  $refId=$_REQUEST['refIdComment'];
}


$commentImputation='';
if (pq_array_key_exists('commentImputation',$_REQUEST)) {
...
<?php 
  ...
  $commentImputationNote=false;
  if (pq_array_key_exists('commentImputationNote',$_REQUEST)) {
  ...
  $finalComment = '['.date("Y-m-d H:i:s").']: '.$commentImputation;
  $assignment=new Assignment($idAssignment);
  $assignment->comment=$finalComment."\n\n".$assignment->comment;
  Assignment::$_skipRightControl=true;
  $resSave=$assignment->save();
  ...
  if (getLastOperationStatus($resSave)=='OK') {
    echo $finalComment;
  } else {
    echo $resSave;
  } 
