...
$lstProj = pq_explode(',', RequestHandler::getValue('lstProj'));
...
foreach ($lstProj as $id=>$val){  
  $val=(($mode =='validateCons' or $mode=='cancelCons') and $all=='false')?pq_substr($val,6):$val;
  ...
  foreach ($proectsSubList as $key=>$name){
    ...
    if($mode =='validateCons' or $mode=='cancelCons'){
  ...
  if(($mode =='validateCons' or $mode=='cancelCons') and $all=='false')$lstProj[$id]=$val;
...
//==============//
if($mode =='validateCons'){ // create all consolidationValidation for save 
  $lstImpLocked=array();
...
//     }
    ...
    $cons->monthlyRevenue=RequestHandler::getValue('monthlyRevenue_'.$idproj);
...
if($mode !='validateCons' and $mode!='cancelCons'){
  if($mode=='Locked'){
