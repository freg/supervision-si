...
// Get the filter info
$cancel=false;
if (pq_array_key_exists('cancel',$_REQUEST)) {
  $cancel=true;
}
$clean=false;
if (pq_array_key_exists('clean',$_REQUEST)) {
  $clean=true;
}
$valid=false;
if (pq_array_key_exists('valid',$_REQUEST)) {
  $valid=true;
}
$default=false;
if (pq_array_key_exists('default',$_REQUEST)) {
  $default=true;
}
...
if ($filterObjectClass!='Planning' and $filterObjectClass!='GlobalPlanning' and $filterObjectClass!='VersionsPlanning' and $filterObjectClass!='ResourcePlanning' and pq_substr($filterObjectClass,0,7)!='Report_' and $filterObjectClass!='PlanningWorkPlan' and $filterObjectClass!='ProjectDashboard') Security::checkValidClass($filterObjectClass);
else if (pq_substr($filterObjectClass,0,7)=='Report_') Security::checkValidClass(pq_substr($filterObjectClass,7));

$name="";
if (pq_array_key_exists('filterName',$_REQUEST)) {
  $name=$_REQUEST['filterName'];
  $name=pq_htmlspecialchars($name,ENT_QUOTES,'UTF-8');
}
...
$filterLayout='stockFilterLayout' . $filterObjectClass;
if ($cancel) {
  if (! $comboDetail) {
    if (sessionValueExists($filterName)){
      $user->_arrayFilters[$filterObjectClass]= getSessionValue($filterName);
	    ...
	    if (pq_array_key_exists($filterObjectClass, $user->_arrayFilters)) {
    ...
    if (sessionValueExists($filterName.'_Detail')){
      ...
      if (pq_array_key_exists($filterObjectClass, $user->_arrayFiltersDetail)) {
...
if ($clean or $cancel or $valid) {
    ...
    if (sessionValueExists($filterName)) {
      ...
      unsetSessionValue($filterLayout);
    }
	} else {
	  if (sessionValueExists($filterName.'_Detail')){
...
if ( ! $clean and ! $cancel and !$valid) {
	if (! $comboDetail) {
	  if (pq_array_key_exists($filterObjectClass,$user->_arrayFilters)) {
	   ...
	   setSessionValue($filterLayout,$user->_arrayFilters[$filterObjectClass]);
	  } else {
	    setSessionValue($filterName, array());
	    setSessionValue($filterLayout, array());
	  }
	} else {
    if (pq_array_key_exists($filterObjectClass,$user->_arrayFiltersDetail)) {
