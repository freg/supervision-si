...
if (! pq_array_key_exists ( 'objectClassName', $_REQUEST )) {
...
if (pq_array_key_exists ( 'confirmed', $_REQUEST )) {
...
IdentificationTags::transformObjectMentions($newObj);

// If user explicitly set project or parent activity, cancel moveToAfterCreate #11344
if (RequestHandler::isCodeSet('moveToAfterCreate')) {
  if (pq_array_key_exists('idProject_detail', $_REQUEST) && $_REQUEST['idProject_detail'] != '') {
    RequestHandler::unsetCode('moveToAfterCreate');
    if (isset($newObj->Origin)) $newObj->Origin->_moveToAfterCreate = null;
    $pe = $className . 'PlanningElement';
    if (isset($newObj->$pe)) $newObj->$pe->_moveToAfterCreate = null;
  }
}


if($className=='VotingAttributionRule' or $className=='VotingUseRule' or $className=='VotingUseRulePerProject'){
  $newObj->refType = null;
  $refType = RequestHandler::getValue('_spe_refType');
  $votingItem = new VotingItem();
  if($votingItem->isVotableClass($refType)){
    $newObj->refType = $refType;
  }
}
if ($newObj->id == '0') {
  $newObj->id = null;
...
if (property_exists($newObj, 'idContact') and pq_array_key_exists('idAffectable',$_REQUEST) ){
  $newObj->idContact=RequestHandler::getValue('idAffectable');
}

$oldObjectMentions = array();
if ($newObj->id) {
  $oldObjectForMentions = new $className($newObj->id);
  $oldObjectMentions = IdentificationTags::extractObjectMentionTargets($oldObjectForMentions);
}
$newObjectMentions = IdentificationTags::extractObjectMentionTargets($newObj);
$objectMentionsToProcess = array_diff_key($newObjectMentions, $oldObjectMentions);

$result='';
$isStop=false;

//control for checklist in function control on sqlElement
// if (pq_array_key_exists('checklistDefinitionId',$_REQUEST) and pq_array_key_exists('checklistId',$_REQUEST)) {
//   $included=true;
...
if($isStop==false){
    ...
    if(get_class ( $newObj )=='Activity' and RequestHandler::isCodeSet('selectedResource')){
      ...
      $result = $newObj->save ();
      $checkObj = new $className($newObj->id);
      if (property_exists($checkObj, 'description')) {
      }
    }
  
  // Check if checklist button must be displayed
  $crit = "nameChecklistable='" . get_class ( $newObj ) . "'";
  $type = SqlElement::getTypeName(get_class ( $newObj ));
  ...
  if ($status == "OK" and $newObj->id and count($objectMentionsToProcess) > 0) {
    IdentificationTags::processMentionActions($objectMentionsToProcess, $newObj, $className, $newObj->id);
  }
  if ($status == "OK" and $className=='Project') {
    ...
    if($newObj->idCalendarDefinition!=$obj->idCalendarDefinition){
      echo '<input type="hidden" id="idProjectForCalendarRefresh" value="'.$newObj->id.'" />';
      echo '<input type="hidden" id="idCalendarForCalendarRefresh" value="'.$newObj->idCalendarDefinition.'" />';
    }
  }
  if ($status == "OK" && $className == 'Resource') {
    if (isset($obj->isMaterial) && isset($newObj->isMaterial) && $obj->isMaterial != $newObj->isMaterial) {
      if ($newObj->isMaterial == "1") {
        echo '<input type="hidden" id="forceGotoAfterSave" value="true" />';
        echo '<input type="hidden" id="forceGotoAfterSaveClass" value="ResourceMaterial" />';
        echo '<input type="hidden" id="forceGotoAfterSaveId" value="' . htmlEncode($newObj->id) . '" />';
      } else {
        echo '<input type="hidden" id="forceGotoAfterSave" value="true" />';
        echo '<input type="hidden" id="forceGotoAfterSaveClass" value="Resource" />';
        echo '<input type="hidden" id="forceGotoAfterSaveId" value="' . htmlEncode($newObj->id) . '" />';
      }
    }
  }
  ...
  if (pq_array_key_exists('joblistDefinitionId',$_REQUEST)) {
    $included=true;
...
function formatResult($result) {
  if (pq_array_key_exists ( 'comboDetail', $_REQUEST )) {
    $res = $result;
    $res = pq_str_replace ( '"lastOperationStatus"', '"lastOperationStatusComboDetail"', $res );
    $res = pq_str_replace ( '"lastSaveId"', '"lastSaveIdComboDetail"', $res );
    $res = pq_str_replace ( '"lastOperation"', '"lastOperationComboDetail"', $res );
    return $res;
  } else {
    return $result;
  }
...
if (isset($status) and $status=='OK') {
  // Give the response to the browser now, then finish processing this user's deferred
  // consolidations (ex: Organization::updateSynthesis) in the background of this same request.
  WaitingUpdate::flushAndExecuteWaiting();
}
?>
