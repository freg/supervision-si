...
if (pq_array_key_exists('pluginFile',$_FILES)) {
...
} else if (pq_array_key_exists('pluginFile',$_FILES) and pq_array_key_exists('name',$_FILES['pluginFile'])) {
...
// PBER #11467 - Unzip only if file contains other zips (will be extracted with Plugin::load();
if (! $error) {
  $hasZip=false;
  $hasOnlyZip=true;
  $za = new ZipArchive();
  ...
  for( $i = 0; $i < $za->numFiles; $i++ ){
    $stat = $za->statIndex($i);
    if (pq_substr($stat['name'] ,-4)=='.zip')  $hasZip=true;
    else $hasOnlyZip=false;
  }
  if ($hasZip and $hasOnlyZip) {
    ...
    $za->close();
    if ($hasZip and ! $hasOnlyZip) {
      $error = htmlGetErrorMessage("Plugin $uploadfile contains zip file and unexpected extra file<br/> => the plugin has not been extracted");
      $message="<div class='messageError' >" . $error . "</div>"
          ."<input type='hidden' value='resultERROR' />";
      kill($uploadfile);
    }
