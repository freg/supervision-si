...
<?php   if ($line->check01) {?>
				...
				<?php 
				      ...
				      for ($i=1;$i<=5;$i++) {
								...
								$title='title0'.$i;
								$value='value0'.$i;?>
								<td style="min-width:100px;width:<?php echo $width;?>%;vertical-align:top;float:left;<?php if (!$line->$check) echo 'display:none';?>" title="<?php echo ($print)?'':$line->$title;?>" >
					<?php if ($line->$check) {
		                ...
		                echo '<img src="../view/img/' . $checkImg . '" />&nbsp;'.htmlEncode($line->$check).'&nbsp;&nbsp;';
							    } else {?>
								  <div dojoType="dijit.form.CheckBox" type="checkbox"
						        <?php if ($line->exclusive and ! $print) {?>onClick="checkClick(<?php echo $line->id;?>, <?php echo $i;?>);formChangeInProgress=true;" <?php }?>
            ...
            maxlength="4000"
            class="input" onChange="formChangeInProgress=true;"><?php echo $lineVal->comment;?></textarea>
          <?php } else {
      ...
      <tr>
	    ...
	    <tr>
	      ...
	      <?php if (! $print) {?>
            ...
            maxlength="4000"
            class="input" onChange="formChangeInProgress=true;"><?php echo $checklist->comment;?></textarea>
          <?php } else {
