...
if ( pq_array_key_exists('report',$_REQUEST) ) {
	...
	if (pq_array_key_exists('idProject',$_REQUEST) and pq_trim($_REQUEST['idProject'])!="") {
	  Security::checkValidId(pq_trim($idProject['format']));
...
if ( ! ( $accessRightRead!='ALL' or (sessionValueExists('project') and getSessionValue('project')!='*') and pq_strpos(getSessionValue('project'), ",") === null)
  ...
  $idRes='';
    ...
    echo ' "items":[';
		...
		foreach ($list as $line) {
			...
			//add expanded status
			if (($line['reftype']=='Resource' or $line['reftype']=='Project') and pq_array_key_exists('Planning_'.$line['reftype'].'_'.$line['refid'], $collapsedList)) {
				echo ',"collapsed":"1"';
			} else {
				echo ',"collapsed":"0"';
			}
			...
			foreach ($depList as $dep) {
				if ( isset($arrayPeAss[$dep->predecessorId])) {
					foreach($arrayPeAss[$dep->predecessorId] as $assId) {
						...
						if (pq_array_key_exists($assId,$arrayResource[$idResource])) {
							$listPred.=($listPred!="")?',':'';
							$listPred.=$assId;
						}
					}
