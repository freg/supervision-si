...
<?php if ($isIE and $isIE<=9) {?>
  action="../tool/saveAttachment.php?isIE=<?php echo ($isIE?$isIE:'');?>&csrfToken=<?php echo getSessionValue("Token"); ?>"
  target="resultPost"
...
<?php }?> 
  >
    ...
    <input id="attachmentMode" name="attachmentMode" type="hidden" value="<?php echo $attachmentMode;?>" />
    <?php if($attachmentMode==false){
      $attachmentMode = $_POST['attachmentMode'] ?? $_GET['attachmentMode'] ?? '';
    }else if($attachmentMode=="profilePic"){
      $param="attachmentMode=profilePic";
    }
    ?>
        ...
        <tr height="30px">
          ...
          <span id="attachmentFileDropArea" >  
           <input MAX_FILE_SIZE="<?php echo Parameter::getGlobalParameter('paramAttachmentMaxSize');?>"
            ...
            url="../tool/saveAttachment.php?<?php echo$param.Security::addTokenIndexToUrl(''); ?>"
            <?php if (! $isIE) {?>
            ...
            label="<?php echo i18n("buttonBrowse");?>"
            title="<?php echo i18n("helpSelectFile");?>"  />
            <span style="font-style:italic;position: absolute; z-index: 49; top: 8px; left: 120px; color: #AAAAAA; width:230px;pointer-events: none;"><?php echo i18n("dragAndDrop");?></span>
            </span>
          <?php }?>
