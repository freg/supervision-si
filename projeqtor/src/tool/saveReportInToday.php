...
// Get the note info
if (! pq_array_key_exists('reportId',$_REQUEST)) {
...
$item->staticSection='Report_'.$reportId;
$item->idle=0;
...
foreach ($params as $pName=>$pValue) {
	...
	if (is_array($reqValue)) $reqValue=implode(',', $reqValue);
	if (pq_trim($reqValue)!=pq_trim($pValue)) {
