...
if (! isset($obj)) {
	...
	$objectClass=Security::checkValidClass($objectClass);
...
if (isset($obj)) {
  ...
  ?>
  <?php  if (property_exists($obj, 'idStatus') and $displayWidthButtonCI>=500 and get_class($obj)!='Mail' and get_class($obj)!='InputMailboxTicket') {
// Bug correction
// Now take care of fieldAttribute
  ...
  <div style="float:left;display:table-cell ;top:0px;width:130px;height:35px;vertical-align:middle;position:relative;z-index:99998;">
    <div style="white-space:normal;width:133px;max-width:133px;<?php echo (isFF())?'height:35px;':'top:-1px;height:40px;';?>max-height:40px;display:table-cell;padding:0px 4px;vertical-align: middle;zoom:0.9;-moz-transform: scale(0.9);overflow:hidden;position:absolute;<?php if ($updateRight and $canUpdateStatus and $obj->id) echo "cursor:pointer;";?>"
    <?php if ($updateRight and $canUpdateStatus and $obj->id) {?> onClick="showDirectChangeStatus();" title="<?php echo i18n('moveStatusBar');?>" <?php }?> >
    <?php if ($obj->idStatus) {
    	$status=new Status($obj->idStatus);
    	echo colorNameFormatter($status->name."#split#".$status->color);
    }?>
    </div>
    <div class="statusBar" tabindex="0" id="directChangeStatusDiv" style="<?php echo (isFF())?'top:-15px;':'';?>white-space:normal;display:none;position:absolute;width:133px;zoom:0.9; -moz-transform: scale(0.9);padding:0px 4px 4px 4px;" onfocusout="hideElementOnFocusOut(this,null)">
      <?php 
...
// MTY - LEAVE SYSTEM      
  ...
  <?php
  $hiddenLastUpdate =(property_exists ( $obj, 'lastUpdateDateTime' ))?$obj->getExtraHiddenFields($objectClass):array();
  if (property_exists ( $obj, 'lastUpdateDateTime' ) && $obj->lastUpdateDateTime && !in_array('lastUpdateDateTime', $hiddenLastUpdate)) {
    echo formatDateThumb(null,$obj->lastUpdateDateTime,'left',32,'Update');
  }
  ?>
  </div>
  <div style="padding-right:16px;float:left;<?php if($displayWidthButtonCI<800) echo 'display:none;'?>" <?php echo ($canUpdateCreationInfo)?'class="buttonDivCreationInfoEdit" onClick="changeCreationInfo();"':'';?>>
  <?php 
  if (!$comboDetail and $obj->id and property_exists($obj,'idUser') and get_class($obj)!='Affectation') {
    $hiddenFields = $obj->getExtraHiddenFields($objectClass);
    if (!in_array('idUser', $hiddenFields)){
    ...
    $creationDate='';
  	...
  	if ($creationDate && !in_array('creationDate', $hiddenFields) && !in_array('creationDateTime', $hiddenFields )) {
