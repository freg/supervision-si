...
if (! pq_array_key_exists('linkObjectClassName',$_REQUEST)) {
  throwError('linkObjectClassName parameter not found in REQUEST');
}

if (! pq_array_key_exists('mainObjectClass',$_REQUEST)) {
...
$selected = RequestHandler::getValue('selected');
$selected = (pq_strpos($selected, '_'))?explode('_', $selected):$selected;

$list =  getUserVisibleObjectsList($linkObjectClassName, true, 'List', 'id'.$mainObjectClass);
if(is_array($selected)){
  foreach ($selected as $idObject){
    $list[$idObject]=SqlList::getNameFromId($linkObjectClassName, $idObject);
  }
}else if($selected){
  $list[$selected]=SqlList::getNameFromId($linkObjectClassName, $selected);
}
?>
...
class="selectList" >
 ...
 foreach ($list as $key => $name) {
   $isSelected = ($selected and ((is_array($selected) and in_array($key, $selected)) or ($selected == $key)))?"selected='selected'":"";
   echo "<option $isSelected value='$key'>#".htmlEncode($key)." - ".htmlEncode($name)."</option>";
 }
 ?>
</select>
