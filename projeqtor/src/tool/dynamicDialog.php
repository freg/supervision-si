...
if (! pq_array_key_exists('dialog', $_REQUEST)) {
	throwError('dialog parameter not found in REQUEST');
}
$dialog=$_REQUEST['dialog'];

$dialog=Security::checkValidAlphanumeric($dialog);
if (pq_strtolower(pq_substr($dialog,0,6))!='dialog' and pq_strtolower(pq_substr($dialog,0,4))!='list') {
  traceHack("dynamicDialog called with not allowed dialog parameter '$dialog'");
}
if ( ($dialog=="dialogLogfiles" or $dialog=="dialogLogfile") and ! (isset($paramNewGuiSwitch) and $paramNewGuiSwitch) ) {
  Security::checkDisplayMenuForUser('Admin');
}
$dialogFile="../tool/dynamic".pq_ucfirst($dialog).'.php';
