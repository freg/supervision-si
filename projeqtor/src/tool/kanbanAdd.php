...
if (! pq_array_key_exists('kanbanName',$_REQUEST)) {
  throwError('Parameter kanbanName not found in REQUEST');
}
$name=$_REQUEST['kanbanName'];

if (! pq_array_key_exists('kanbanReffList',$_REQUEST)) {
  throwError('Parameter kanbanReffList not found in REQUEST');
}
$typeData=$_REQUEST['kanbanReffList'];

if (! pq_array_key_exists('kanbanTypeList',$_REQUEST)) {
  throwError('Parameter kanbanTypeList not found in REQUEST');
}
$type=$_REQUEST['kanbanTypeList'];

if (! pq_array_key_exists('shared',$_REQUEST)) {
...
if($type=='Status'){
  ...
  $kanban->param='{"column":[{"from":"n","name":"'.pq_ucfirst(i18n('undefinedValue')).'","cantDelete":true}],"typeData":"'.$typeData.'"}';
}
$kanban->save();
echo $kanban->id;
?>