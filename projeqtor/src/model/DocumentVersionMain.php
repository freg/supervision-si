...
// ============================================================================**********
  ...
  public function control(){
    $result="";
    $critWhere="idDocument='". Sql::fmtId($this->idDocument) . "' and name='" . Sql::fmtStr($this->name) . "'";
    if ($this->id) {
  ...
  function save($fromDoc=false) {
    $mode="";
  	...
  	if ($doc->documentReference) {
  	  $this->fullName=$doc->documentReference.pq_str_replace('{VERS}',$this->name,$suffix);
  	} else {
  	...
  	$pos=pq_strpos($this->fileName,'.');
  	if ($pos) {
  	  $this->fullName.=pq_substr($this->fileName,$pos);
  	}
  	$this->fullName=pq_substr($this->fullName,0,$this->getDataLength('fullName'));
  ...
  function getUploadFileName() {
    ...
    $fileName= $this->fileName ? $this->fileName : 'noFileNameAdmitted';
     $ext = pq_strtolower ( pathinfo ( $fileName, PATHINFO_EXTENSION ) );
//     if (pq_substr($ext,0,3)=='php' or pq_substr($ext,0,4)=='phtm'  or $ext=='phar' or $ext=='pgif') {
//     	$fileName.='.projeqtor.txt';
//     }
//    Previous replace now done in Security::checkValidFileName(xx,xx,true)
     $fileName=Security::checkValidFileName($fileName,true, true);
