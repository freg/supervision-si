...
/** =========================================================================
  ...
  public function control(){
    ...
    if ($this->id and $this->id==$this->idTestSession) {
      ...
      if (pq_array_key_exists('#' . $this->TestSessionPlanningElement->id,$parentList)) {
        $result.='<br/>' . i18n('errorHierarchicLoop');
      }
    }
  ...
  public function save() {
    ...
    TestCaseRun::$_skipUpdateDependencies=true;
    $result=parent::save();
  	self::$_doNotUpdateDependencies=false;
  	TestCaseRun::$_skipUpdateDependencies=false;
  ...
  public function copyTo($newClass, $newType, $newName, $newProject, $setOrigin, $withNotes, $withAttachments, $withLinks, $withAssignments = false, $withAffectations = false, $toProject = null, $toActivity = null, $copyToWithResult = false, $copyToWithActivityPrice=false, $copyToWithStatus = false, $copyToWithSubTask=false, $moveAfterCreate=null) {
    $newObj=parent::copyTo($newClass, $newType, $newName, $newProject, $setOrigin, $withNotes, $withAttachments, $withLinks, $withAssignments, $withAffectations, $toProject, $toActivity, $copyToWithResult, $copyToWithActivityPrice, $copyToWithStatus, false, $moveAfterCreate);
    $new=$this->copyTestCasRun($newObj);
    return $new;
  }
  ...
  public function updateDependencies() {
    if (self::$_doNotUpdateDependencies) return;
  	...
  	$this->countPlanned=0;
  	$this->countTotal=0;
  	if ($this->id and count($this->_TestCaseRun)==0 ) {
  	  $tcr=new TestCaseRun();
  	  $this->_TestCaseRun=$tcr->getSqlElementsFromCriteria(array('idTestSession'=>$this->id));
  	}
  	...
  	if ($this->id and count($this->_Link)==0 ) {
  	  $tcr=new Link();
  	  $this->_Link=$tcr->getSqlElementsFromCriteria(array('ref1Type'=>'TestSession', 'ref1Id'=>$this->id));
  	}
   ...
   public function getCalculatedItem(){
     ...
     if ($this->id) {
       ...
       $this->runStatusIcon='<img style="position:absolute;" src="../view/css/images/icon'.pq_ucfirst($name).'22.png" />&nbsp;&nbsp;&nbsp;';
     }
  }
