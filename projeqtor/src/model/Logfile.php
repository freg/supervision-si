...
class Logfile
{
    ...
    function getInfos() {
      if ($this->name) {
        ...
        if (is_file($filepath) and pq_substr($this->name,0,1)!='.' and pq_substr($this->name,0,1)!='/' 
            and pq_substr($this->name,-4)=='.log') {
    ...
    static function getList($numericIndexes=false) {
      ...
      while (!$error and ($file = readdir($handle)) !== false) {
        ...
        if ($file=='critRes_allData.log') { continue;}
        if (is_file($filepath) and pq_substr($file,0,1)!='.') {
