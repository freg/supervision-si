...
class ImputationValidation {
  ...
  static function drawUserWorkList($idUser, $idTeam, $idOrga, $startDay, $endDay, $idProject) {
    ...
    $showError=getSessionValue('showErrorWork');
    ...
    if ($idProject!=null) {
      ...
      if (empty($pAff)) {
        ...
        foreach ($pAff as $id=>$val) {
          if (pq_array_key_exists($val->idResource, $tabToSearch)) {
    ...
    $result.='     <td style="border: 1px solid grey;border-right: 1px solid white;height:60px;width:16%;text-align:center;vertical-align:middle;">' . i18n('Resource') . '</td>';
    $result.='     <td style="border: 1px solid grey;border-right: 1px solid white;height:60px;width:10%;text-align:center;vertical-align:middle;">' . i18n('colWeek') . '</td>';
    $result.='     <td style="border: 1px solid grey;border-right: 1px solid white;height:60px;width:8%;text-align:center;vertical-align:middle;">' . i18n('expectedWork') . '</td>';
    $result.='     <td style="width:20%;border: 1px solid grey;border-right: 1px solid white;">';
    $result.='      <table width="100%"><tr><td colspan="3" style="height:30px;text-align:center;vertical-align:middle;">' . i18n('inputWork') . '</td></tr>';
    $result.='      <tr><td style="border-top: 1px solid white;border-right: 1px solid white;width:33%;height:30px;text-align:center;vertical-align:middle;">' . i18n('operationalWork') . '</td>';
    $result.='      <td style="border-top: 1px solid white;border-right: 1px solid white;width:33%;height:30px;text-align:center;vertical-align:middle;">' . i18n('administrativeWork') . '</td>';
    $result.='      <td style="border-top: 1px solid white;border-right: 1px solid white;width:33%;height:30px;text-align:center;vertical-align:middle;">' . i18n('sum') . '</td></tr></table>';
    $result.='     </td>';
    $result.='     <td style="border: 1px solid grey;border-right: 1px solid white;height:60px;width:20%;text-align:center;vertical-align:middle;">' . i18n('ImputationSubmit') . '</td>';
    $result.='     <td colspan="2" style="border: 1px solid grey;height:60px;width:26%;text-align:center;vertical-align:middle;">';
    $result.='       <table width="100%"><tr><td width="62%">' . i18n('menuImputationValidation') . '</td>';
    $result.='       <td width="30%">';
    $result.='       <span class="mediumTextButton" id="buttonValidationAll" style="width:98% !important;" type="button" dojoType="dijit.form.Button" showlabel="true">' . i18n('validateWorkPeriod') . '         <script type="dojo/method" event="onClick" >' . '           validateAllSelection();' . '         </script>' . '       </span></td>';
    $result.='        <td width="8%" style="padding-left:5px;padding-right:5px;">
	                     <div title="' . i18n('buttonSelectAll') . '" dojoType="dijit.form.CheckBox" type="checkbox" 
    ...
    if (!isset($noResource)) {
      ...
      if (isset($userVisbileResourceList)) {
        ...
        foreach ($userVisbileResourceList as $idResource=>$name) {
          ...
          $startWeekTmp=$startWeek;
          if ($startDay and $res->startDate>$startDay) {
            $startWeekTmp=getWeekNumberFromDate($res->startDate);
          }
          if (!isset($periodValueListSorted[$startWeekTmp]) and !isset($periodValueListOutOfScope[$startWeekTmp])
              and (!$endWeek or $startWeekTmp<=$endWeek)) {
            if ($showSubmitted!=='1' and $showValidated!=='1' and ! $res->idle and (! $res->endDate or $res->endDate<$startDay)) {
              $startWorkPeriod=new WorkPeriod();
              $startWorkPeriod->periodRange='week';
              $startWorkPeriod->periodValue=$startWeekTmp;
              $startWorkPeriod->idResource=$idResource;
              $startWorkPeriod->validated=0;
              $startWorkPeriod->submitted=0;
              $periodValueListSorted[$startWeekTmp]=$startWorkPeriod;
              array_unshift($periodValueList,$startWorkPeriod);
          ...
          if ($showError==='1' or $showError==='0') {
            foreach ($periodValueListSorted as $idWeek=>$week) {
              ...
              $isError=($inputTotal==$expected)?false:true;
              if ($showError==='1'&&!$isError) unset($periodValueListSorted[$idWeek]);;
              if ($showError==='0'&&$isError) unset($periodValueListSorted[$idWeek]);;
            }
          }
          foreach ($periodValueListSorted as $idWeek=>$week) {
            $uniqueId=($week->id)?$week->id:$week->periodValue . "_" . $week->idResource;
            ...
            if ($showError==='1'&&!$isError) continue;
            if ($showError==='0'&&$isError) continue;
            $idCheckBox++;
            ...
            $goto="saveDataToSession('userName',$idResource,false, function() {
                     ...
                     saveDataToSession('monthSpinner'," . intval(pq_substr($firstDay, 5, 2)) . ",false, function() {
            ...
            $result.='   <tr>';
            if ($countWeek==0) {
              $result.='     <td rowSpan="' . count($periodValueListSorted) . '" style="vertical-align:top;padding-top:5px;border: 1px solid grey;height:30px;width:16%;text-align:left;vertical-align:middle;background:white;">';
            ...
            $displayWeek='<span style="white-space:nowrap;">' . $weekValue . '</span> <span style="font-size:80%;font-style:italic;white-space:nowrap;">(' . htmlFormatDate($firstDay) . ' - ' . htmlFormatDate($lastDay) . ')</span>';
            $result.='     <td onClick="' . $goto . '" style="cursor:pointer;border: 1px solid grey;height:30px;width:10%;text-align:center;vertical-align:middle;background:white;">' . $displayWeek . '</td>';
            $result.='     <td onClick="' . $goto . '" style="cursor:pointer;border: 1px solid grey;height:30px;width:8%;text-align:center;vertical-align:middle;background:white;">' . $expected . '</td>';
            $result.='     <td onClick="' . $goto . '" style="cursor:pointer;width:23%;border: 1px solid grey;">';
            $result.='      <table style="width:100%;height:35px">';
            $result.='        <tr><td style="background:white;border-right: 1px solid grey;width:33%;height:30px;text-align:center;vertical-align:middle;">' . $inputWork . '</td>';
            $result.='        <td style="background:white;border-right: 1px solid grey;width:33%;height:30px;text-align:center;vertical-align:middle;">' . $inputAdm . '</td>';
            $result.='        <td style="background:white;' . $backgroundColor . 'width:33%;height:30px;text-align:center;vertical-align:middle;">' . $inputTotal . '</td></tr>';
            $result.='      </table>';
            $result.='     </td>';
            $result.='   <td style="border: 1px solid grey;height:30px;width:20%;text-align:left;vertical-align:middle;background:white;">';
            $result.='   <div id="submittedDiv' . $uniqueId . '" name="submittedDiv' . $uniqueId . '" width="100%" dojoType="dijit.layout.ContentPane" region="center">';
            if ($week->submitted) {
              $result.='     <table width="100%"><tr><td style="height:30px;">' . formatIcon('Submitted', 32, i18n('submittedWork', array(
                  $name,
                  htmlFormatDate($week->submittedDate))), false, true) . '</td>';
              $result.='     <td style="width:73%;padding-left:5px;height:30px;">' . i18n('submittedWork', array(
                  $name,
                  htmlFormatDate($week->submittedDate))) . '</td>';
              $result.='     <td style="width:27%;height:30px;padding-right:8px;">';
              $result.='      <span id="buttonCancel' . $week->id . '" style="width:100px;height:25px" class="mediumTextButton" type="button" dojoType="dijit.form.Button" showlabel="true">' . i18n('buttonCancel') . '<script type="dojo/method" event="onClick" >' . '        saveImputationValidation(' . $week->id . ', "cancelSubmit");' . '        saveDataToSession("idCheckBox", ' . $idCheckBox . ', false);' . '       </script>' . '     </span>';
            ...
            $result.='   <td style="border: 1px solid grey;height:30px;width:26%;text-align:left;vertical-align:middle;background:white;">';
            ...
            if ($week->validated) {
              ...
              $result.='     <td style="height:30px;">' . formatIcon('Submitted', 32, i18n('validatedLineWorkPeriod', array(
                  $locker,
                  htmlFormatDate($week->validatedDate))), false, true) . '</td>';
              $result.='     <td style="width:' . $size . ';padding-left:5px;height:30px;">' . i18n('validatedLineWorkPeriod', array(
                  $locker,
                  htmlFormatDate($week->validatedDate))) . '</td>';
              if ($canValidate) {
                $result.='     <td style="width:27%;padding-right:8px;">';
                $result.='      <span id="buttonCancelValidation' . $week->id . '" style="width:100% !important; " class="mediumTextButton" type="button" dojoType="dijit.form.Button" showlabel="true">' . i18n('buttonCancel') . '       <script type="dojo/method" event="onClick" >' . '        saveDataToSession("idCheckBox", ' . $idCheckBox . ', false);' . '        saveImputationValidation(' . $week->id . ', "cancelValidation");' . '       </script>' . '     </span>';
              }
            } else {
              ...
              if ($canValidate) {
                $result.='     <td style="width:27%;padding-right:8px;">';
                $result.='      <span class="mediumTextButton" id="buttonValidation' . $uniqueId . '" style="width:100% !important;" type="button" dojoType="dijit.form.Button" showlabel="true">' . i18n('validateWorkPeriod') . '       <script type="dojo/method" event="onClick" >' . '        saveImputationValidation("' . $uniqueId . '", "validateWork");' . '        saveDataToSession("idCheckBox", "' . $uniqueId . '", false);' . '       </script>' . '     </span>';
              }
              // } else {
    ...
    if (!$idCheckBox or $noData==true or isset($noResource)) {
