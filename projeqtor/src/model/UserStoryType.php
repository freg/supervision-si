...
class UserStoryType extends Type {
  ...
  private static $_databaseCriteria = array('scope'=>'UserStory');
  ...
  function __construct($id = NULL, $withoutDependentObjects=false) {
    ...
    if (!$this->idWorkflow) {
      $wf=new Workflow();
      $wfList=$wf->getSqlElementsFromCriteria(null,true," name like '%Agile%'");
      if (count($wfList)>0) {
        $wf=reset($wfList);
        $this->idWorkflow = $wf->id;
      }
      self::$_fieldsAttributes['idWorkflow']='required';
    }
  }
