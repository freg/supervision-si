  ...
  public function control(){
    ...
    if(! $this->id) { // New note
      ...
      if (property_exists($class, 'idle')){
        ...
        $obj = new $class($id);
        if ($obj->idle and $canAddNoteClosedElement->rightAccess!='1') {
          $result .= "<br/>" . i18n("errorAddOnClosedItem").'<br/><span style="font-size:80%">'.i18n("Note").'</span>';
        }
      } 
    }
    // Control rights to update item
    $class = $this->refType;
    $id = $this->refId;
    $obj = new $class($id);
    $userNote = $user->getProfile($obj);
    $canUpdate=(securityGetAccessRightYesNo('menu' . $class, 'update', $obj)=='YES');
    ...
    if (!$canUpdate and $canAddNoteReadOnlyElement->rightAccess!='1') {
