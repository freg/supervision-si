...
// MTY - GENERIC DAY OFF      
      "dayOfWeek0" => "hidden,forceExport",
      "dayOfWeek1" => "hidden,forceExport",
      "dayOfWeek2" => "hidden,forceExport",
      "dayOfWeek3" => "hidden,forceExport",
      "dayOfWeek4" => "hidden,forceExport",
      "dayOfWeek5" => "hidden,forceExport",
      "dayOfWeek6" => "hidden,forceExport",
// MTY - GENERIC DAY OFF      
      "sortOrder"=>"hidden"      
      );
...
// MTY - GENERIC DAY OFF
    ...
    if (Parameter::getGlobalParameter('OpenDayMonday')=='offDays') {
        $this->dayOfWeek1 = 1;
        self::$_fieldsAttributes["dayOfWeek1"] = "readonly,hidden,forceExport";        
    }
    if (Parameter::getGlobalParameter('OpenDayTuesday')=='offDays') {
        $this->dayOfWeek2 = 1;
        self::$_fieldsAttributes["dayOfWeek2"] = "readonly,hidden,forceExport";                
    }
    if (Parameter::getGlobalParameter('OpenDayWednesday')=='offDays') {
        $this->dayOfWeek3 = 1;
        self::$_fieldsAttributes["dayOfWeek3"] = "readonly,hidden,forceExport";                
    }
    if (Parameter::getGlobalParameter('OpenDayThursday')=='offDays') {
        $this->dayOfWeek4 = 1;
        self::$_fieldsAttributes["dayOfWeek4"] = "readonly,hidden,forceExport";        
    }
    if (Parameter::getGlobalParameter('OpenDayFriday')=='offDays') {
        $this->dayOfWeek5 = 1;
        self::$_fieldsAttributes["dayOfWeek5"] = "readonly,hidden,forceExport";        
    }
    if (Parameter::getGlobalParameter('OpenDaySaturday')=='offDays') {
        $this->dayOfWeek6 = 1;
        self::$_fieldsAttributes["dayOfWeek6"] = "readonly,hidden,forceExport";        
    }
// MTY - GENERIC DAY OFF    
  }
...
// MTY - GENERIC DAY OFF
  ...
  private function drawBankOffDays() {
    ...
    foreach ($calBankOffDaysList as $obj) {
        ...
        if ($readOnly!="readonly " and !$print) {
            // Edit
            $theName = pq_str_replace("'", "\'", $obj->name);
  ...
  private function drawDayOffWeek() {
    ...
    for ($i=0;$i<=6;$i++) {
        ...
        if ($readOnlyObj=="") {
            if (pq_strpos($this->getFieldAttributes($itemName),"readonly")!==false) {
                $readOnly = "readonly ";
            } else {
                $readOnly = "";
            }
...
// MTY - MULTI CALENDAR  
    ...
    public function updateCookiesForCalendar($onlyForUser=false,$idCalDef=null) {
      if (SqlElement::isDeleteConfirmed() and SqlElement::isSaveConfirmed()) return; // comes from test/testAllOnjects.php
...
// MTY - LEAVE SYSTEM
    private function updateLeaveLeaveEarnedPlannedWork($result,$dayOfWeekChanged) {
        ...
        if ($employees) {
            ...
            $employeeIdIn = pq_substr($employeeIdIn, 0,-1).")";                
        } else {
            $employeeIdIn = "";
        }
        ...
        if ($employeeIdIn!="") {
            ...
            foreach($lvRq as $leave){
                ...
                while($theDate <= $leave->endDate) {                    
                    ...
                    if (pq_array_key_exists($dayOfWeek, $dayOfWeekChanged)) {
                ...
                if (count($dateChanged)>0) {
                    ...
                    if ($plWorkInDate!="") { $plWorkInDate = pq_substr($plWorkInDate, 0,-1).")"; }
                    if ($plWorkInDate!="") {
                    ...
                    foreach($dateChanged as $date => $offDay) {
                        if ($offDay==0) {
                            $theDay=pq_substr($date,0,4).pq_substr($date,5,2).pq_substr($date,8,2);
                            if (pq_strpos($offDayCalendar,$theDay)!==false) {continue;} 
