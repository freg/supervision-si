...
// ============================================================================**********
  ...
  public function setAttributes() {
    // Before the early return below, which leaves this method when the user menu
    // is missing : the section must be hidden either way.
    if (! Module::isModuleActive('moduleCrmProspect')) {
      self::$_fieldsAttributes['_sec_loyaltyEvent']='hidden';
      self::$_fieldsAttributes['_spe_ProspectEvent']='hidden';
    }
...
//     }
  ...
  public function drawSpecificItem($item){
    ...
    $result="";
    if ($item=='ProspectEvent') {
      // The contact reads the one field that ties an action to it, whoever owns
      // the action : one created from its client naming it, or one of its own.
      ProspectEvent::drawList($this, array('idContact'=>($this->id??'0')), false);
      return $result;
