...
class NumberFormatter52  {
  ...
  function __construct($locale, $type) {
    ...
    if (false !== setlocale(LC_ALL, $locale . ".UTF-8@euro", $locale . ".UTF-8", $locale) ) {
      ...
      if (pq_strtolower(pq_substr($locale,0,2))=='fr' or pq_strtolower(pq_substr($locale,0,2))=='de') {
    ...
    if ($this->thouthandSeparator and isset($this->thouthandSeparator[0]) and ord($this->thouthandSeparator[0])>127) { // PBER : fix for PHP 8.5
      $this->thouthandSeparator=" ";
    }
  }
  ...
  static function completeKeyDownEvent($colScript) {
    ...
    $result=pq_str_replace($tagEvent,$tagEvent.$evt,$colScript);
    return $result;
  }
}
?>