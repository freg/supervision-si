...
class Plugin extends SqlElement {
    ...
    public function load($file) {
      ...
      if (! is_array($metaData) or ! isset($metaData['pluginVersion'])) {
      ...
      if ($res === TRUE) {
        ...
        for ($i = 0; $i < $zip->numFiles; $i++) {
          ...
          if (pq_substr($entry,0,1)=='/' or pq_substr($entry,0,2)=='..') {
            $result="<b>Try to Hack : incorrect plugin file</b>";
            errorLog("Plugin::load() : Try to Hack : incorrect plugin file - extract out of plugin root");
            errorLog("               : $entry");
            return $result;
          }
          if (pq_strtolower(pq_substr($entry,-4))=='.php') {
            $phpFile = $zip->getFromIndex($i);
            $phpFile=pq_str_replace(array(' ','curl_exec'),'',$phpFile);
            if (pq_strpos($phpFile,'exec(')!==false or pq_strpos($phpFile,'system(')!==false) {
              $result="<b>Try to Hack : incorrect plugin file</b>";
              errorLog("Plugin::load() : Try to Hack : incorrect plugin file - php file contains exec or system");
              errorLog("               : $entry");
              return $result;
            }
          }
        }
        if (! $error) $res=$zip->extractTo(self::getDir());
        $zip->close();
      } 
      if ($res !== TRUE) {
        $result=i18n('pluginUnzipFail', array(self::unrelativeDir($this->zipFile), self::unrelativeDir(self::getDir()) ));
      ...
      PHP_VERSION_ID < 80000 && xml_parser_free($parse); // PBER : PHP8.5 compatibility
        ...
        if ($prop['tag']=='BUTTON') {
          if (isset($prop['attributes']) and is_array($prop['attributes'])) {
            ...
            if ($buttonName and $className and ($scriptJS or $scriptPHP) and $scope) {
              ...
              if ($className!='Planning' and $className!='ResourcePlanning' and $className!='PortfolioPlanning' and $className!='GlobalPlanning'
                  and $className!='Imputation') { // Pseudo Classes
                Security::checkValidClass($className);
              }
    ...
    public function includeFiles() {
      ...
      if (file_exists($jsFile)) {
        echo '<script type="text/javascript" src="'.getStaticFileNameWithCacheMgt($jsFile).'" ></script>';
      }
      // CSS (style sheet)
      $cssFile=$root.'.css';
      if (file_exists($cssFile)) {
        echo '<link rel="stylesheet" type="text/css" href="'.getStaticFileNameWithCacheMgt($cssFile).'" />';
      }
    }
    ...
    public static function checkCustomDefinition($paramClass=null) {
      ...
      foreach ($list as $class) {
        ...
        foreach ($obj->getFieldsList() as $fld) {
          ...
          } else if ($last and ! $obj->isAttributeSetToField($fld,'hidden') and $fld!='_sec_restrictTypes' and $fld!='_spe_restrictTypes'
              and $fld!='_sec_restrictLists' and $fld!='_spe_restrictLists') { // not a specific field, after $last (notes, attachment, link)
            $errorClass[]=$class;
      ...
      if (count($errorClass)==0) {
        ...
        foreach ($errorClass as $class) {
          ...
          if (function_exists('screenCustomizationFixDefinition')) {
            global $availableAttributes, $lastFixedField;
