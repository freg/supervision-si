...
class Sql {
   ...
   * @param $sqlRequest String the resquest to be executed. Can be SELECT, UPDATE, INSERT, DELETE or else
  ...
  static function query($sqlRequest=NULL) {
    ...
    try { 
      ...
      and ( $debugQuery===true or pq_strtolower($debugQuery)==pq_strtolower(pq_substr($sqlRequest,0,pq_strlen($debugQuery) ) ) )  ) {
      ...
      if (self::$dbVersion and self::$dbVersion!='0.0.0') { // we get the version, if not set, may be normal : initial configuration. Must not log error
    ...
    //if (pq_strtoupper(pq_substr($sqlRequest,0,6)=='SELECT')) self::$lastQueryType='SELECT';
    ...
    if (self::$lastQueryType=="UPDATE") {
      if (self::isPgsql()) { // Specific update of sequence in pgsql mode.
      	if (pq_strtolower(pq_substr($sqlRequest,0,11))=='insert into' and !Sql::$maintenanceMode) {
      		$table=pq_substr($sqlRequest,12,pq_strpos($sqlRequest,'(')-13);
      		$seq=pq_trim(pq_strtolower($table)).'_id_seq';
      		if ($result->rowCount()) {
   ...
   * @param $string String the string to be protected
   * @return String the string, protected to ensure a correct sql query
   */
...
//     }   
  ...
  public static function getConnection($forceToReconnect=false) {
    global $cronnedScript, $enforceUTF8;
...
//     	self::$connexion = new PDO($dsn, self::$dbUser, self::$dbPassword,
      ...
      if($cronnedScript){
         if (Cron::$waitDbTimer) traceLog("CRON : cannot connect to DB, wait for DB until ".date('H:i:s',Cron::$waitDbTimer));
         else traceLog("CRON : cannot connect to DB");
      }else{
   ...
   * @return String the version of the DataBase, as String Vx.y.z or empty string if never initialized
   */
  ...
  public static function saveDbRevision($rev) {
    $crit['idUser']=null;
    $crit['idProject']=null;
    $crit['parameterCode']='lastRevisionInstalled';
    $obj=SqlElement::getSingleSqlElementFromCriteria('Parameter', $crit);
    $obj->parameterValue=pq_str_replace('lastRevision=','',$rev);
    $obj->save();
  }
  
  // Retores the Sequence for PgSql
  public static function updatePgSeq($table) {
    if (pq_substr($table,-10)=='tempupdate') return; // Exclude tables that don't have an ID
    Sql::query("SELECT setval('".$table."_id_seq', (SELECT MAX(id) FROM $table));");
  }
  ...
  public static function fmtId($id) {
    if ($id===null or $id=='*' or $id=='' or $id==' ') {
  	  ...
  	  debugTraceLog("Sql::fmtId : id '$id' is not numeric");
  ...
  public static function fmtStr($str) {
  	...
  	$res=self::str($str);
  	$res=pq_substr($res,1,(pq_strlen($res)-2));
  	return $res; 
  }
  ...
  public static function getDbCollation() {
    ...
    if (isset($paramDbCollation) and $paramDbCollation) return $paramDbCollation;
   	...
   	if (Sql::isMysql()) {
   		$value="utf8mb4_general_ci"; // default
