...
class SqlList {
  ...
  public static function getList($listType, $displayCol='name', $selectedValue=null, $showIdle=false, $applyRestrictionClause=false) {
    ...
    if (pq_array_key_exists($listName, self::$list)) {
...
//scriptLog("       =>getListWithCrit($listType, implode('|',$crit), $displayCol, $selectedValue)");
   ...
   * for further needs
   * @param $listType String the name of the table containing the data
   * @return Array an array containing the list of references
   */
...
//scriptLog("fetchList($listType,$displayCol, $selectedValue, $showIdle, $translate)");
    ...
    foreach ($crit as $col => $val) {
      if (get_class($obj)=='Language' and $col=='scope' and pq_substr($obj->getDatabaseTableName(),-13)!='plgcustomlist') continue; // Ticket #6377 - Errors found in logs.
    ...
    if (pq_trim($selectedValue)) {
      if (pq_strpos($selectedValue, ',') !== false){
        $query .= " or " . $obj->getDatabaseColumnName('id') .' IN (' . Sql::fmtStr($selectedValue).')' ;
      }else{
        if ($selectedValue!='*' and $selectedValue!='all' and intval($selectedValue) ) {
          $query .= " or " . $obj->getDatabaseColumnName('id') .'= ' . Sql::fmtStr($selectedValue) ;
        }
      }
    }
    if (property_exists($obj,'_sortCriteriaForList') and pq_trim($obj->_sortCriteriaForList)) {
      ...
      $query .= ' order by ' . $obj->getDatabaseTableName() . '.baselineDate DESC, ' . $obj->getDatabaseTableName() . '.' . $obj->getDatabaseColumnName($displayCol);
    } else {
...
//         	$name=$nameObj->name;
      ...
      if ($displayCol=='name' and property_exists($obj,'_constructForName') and !$calculated ) {
        ...
        foreach ($res as $id=>$name) {
          if (!isset($listNameObj["#$id"])) continue;
        ...
        or pq_strtolower($listType)=='accountable' or pq_strtolower($listType)=='affectable') and $col=='idProject') {
          ...
          if ($val===null or $val=='' or $val=='null') {
            ...
            if ($dataType=='numeric' or $dataType=='decimal' or $dataType=='int' or $dataType=='boolean') {
              if (pq_strpos($val, ',') !== false) $query .= " and " . $obj->getDatabaseTableName() . '.' . $obj->getDatabaseColumnName($col) .' IN (' . Sql::fmtStr($val).')' ;
              else if ($val!=' ') $query .= ' and ' . $obj->getDatabaseTableName() . '.' . $obj->getDatabaseColumnName($col) . '=' . Sql::fmtStr($val);
            } else { 
              $query .= ' and ' . $obj->getDatabaseTableName() . '.' . $obj->getDatabaseColumnName($col) . '=' . Sql::str($val);
            }
          }
...
//           }
      ...
      if ($displayCol=='name' and property_exists($obj,'_constructForName') and !$calculated ) {
        ...
        $listNameObj=$nameObj->getSqlElementsFromCriteria(null,null,"id in ($in)",null, true);
        foreach ($res as $id=>$name) {
          $checkObj=(pq_substr($listType,-7)=='Version')?'Version':$listType;
          $nameObj=$listNameObj["#$id"]??(new $checkObj($id));
          if (method_exists($nameObj, 'setName')) $nameObj->setName();
          $res[$id]=$nameObj->name;
        }
      }
  ...
  public static function getFieldFromId($listType, $id, $field, $translate=true) {
    ...
    if (pq_array_key_exists($id,$list)) {
      $name=$list[$id];
      $obj=new $listType();
...
//       }
  ...
  public static function getStatusList($class) {
    ...
    $wfArray=array();
    //$type=$class.'Type';
    $type=SqlElement::getTypeClassName($class);
