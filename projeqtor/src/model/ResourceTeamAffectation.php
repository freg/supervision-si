...
class ResourceTeamAffectation extends SqlElement {
  ...
  public function control(){
    ...
    if($this->idle==0){
      ...
      if($ress->isMaterial){
        return '<br/>' . i18n('cantAddMaterialToResourcePool');
      }
...
//Gautier #resCap
  ...
  foreach ($resource as $Myres){
    ...
    if(sessionTableValueExist('capacityPeriod', $Myres->id)){
      $tabResCap = getSessionTableValue('capacityPeriod', $Myres->id , array());
  	...
  	foreach ($periods as $p) {
  		...
  		foreach($p['idResource'] as $idP=>$affP) {
  			if (! isset($projects[$idP])) {
  				$cptProj++;
  				$projects[$idP]=array('position'=>$cptProj,
  						'name'=>SqlList::getNameFromId('Affectable',$idP));
  			}
  		}
  	}
    ...
    $nbPeriods = count($periods);
  	...
  	foreach ($periodsPerProject as $idP=>$proj) {
  	  foreach ($proj['periods'] as $p) {
  	    ...
  	    $nameR=$projects[$idP]['name'];
  	    if (is_numeric($projects[$idP]['name'])) $nameR=SqlList::getNameFromId('Affectable', $projects[$idP]['name']);
  	    $result.='['.htmlDisplayNumericWithoutTrailingZeros($p['rate']).']&nbsp;'.$nameR;
  	    $result.= '</div>';
  	    $result.='</div>';
  	  }
  	}
  ...
  public function save () {
    ...
    $res = new Resource($this->idResource,true);
    if( (! $this->endDate and $res->endDate) or  ($res->endDate and $this->endDate > $res->endDate)){
      $this->endDate = $res->endDate;
    }
    if( (!$this->startDate and $res->startDate) or  ($res->startDate and $this->startDate < $res->startDate) ){
