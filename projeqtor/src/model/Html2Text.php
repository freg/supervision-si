...
class Html2Text
{
    ...
    protected function converter(&$text)
    {
        ...
        $text = preg_replace(pq_nvl($this->search), pq_nvl($this->replace), pq_nvl($text));
        $text = preg_replace_callback(pq_nvl($this->callbackSearch), array($this, 'pregCallback'), pq_nvl($text));
        $text = strip_tags($text);
        $text = preg_replace(pq_nvl($this->entSearch), pq_nvl($this->entReplace), pq_nvl($text));
        $text = html_entity_decode($text, $this->htmlFuncFlags, self::ENCODING);

        // Remove unknown/unhandled entities (this cannot be done in search-and-replace block)
        $text = preg_replace('/&([a-zA-Z0-9]{2,6}|#[0-9]{2,4});/', '', pq_nvl($text));

        // Convert "|+|amp|+|" into "&", need to be done after handling of unknown entities
        // This properly handles situation of "&amp;quot;" in input string
        $text = pq_str_replace('|+|amp|+|', '&', $text);
    ...
    protected function buildlinkList($link, $display, $linkOverride = null)
    {
        ...
        $link = ($link)?$link:'';
        // Ignored link types
        if (preg_match('!^(javascript:|mailto:|#)!i', pq_nvl($link))) {
            return $display;
        }

        if (preg_match('!^([a-z][a-z0-9.+-]+:)!i', pq_nvl($link))) {
            $url = $link;
        } else {
            $url = $this->baseurl;
            if (pq_mb_substr($link, 0, 1) != '/') {
                $url .= '/';
            }
            $url .= $link;
        }
    ...
    protected function convertPre(&$text)
    {
        ...
        while (preg_match('/<pre[^>]*>(.*)<\/pre>/ismU', pq_nvl($text), $matches)) {
            ...
            $this->preContent = preg_replace_callback(
                pq_nvl($this->callbackSearch),
                array($this, 'pregCallback'),
                pq_nvl($this->preContent)
            );

            // convert the content
            $this->preContent = sprintf(
                '<div><br>%s<br></div>',
                preg_replace(pq_nvl($this->preSearch), pq_nvl($this->preReplace), pq_nvl($this->preContent))
            );
    ...
    {
        if (preg_match_all('/<\/*blockquote[^>]*>/i', pq_nvl($text), $matches, PREG_OFFSET_CAPTURE)) {
            $originalText = $text;
            $start = 0;
            $taglen = 0;
            $level = 0;
            ...
            foreach ($matches[0] as $m) {
                $m[1] = pq_mb_strlen(pq_substr($originalText, 0, $m[1]));
                if ($m[0][0] == '<' && $m[0][1] == '/') {
                    $level--;
                    if ($level < 0) {
                        ...
                        $body = pq_mb_substr($text, $start + $taglen - $diff, $len);
                        ...
                        $body = preg_replace('/((^|\n)>*)/', '\\1> ', pq_nvl(pq_trim($body)));
                        $body = '<pre>' . pq_htmlspecialchars($body, $this->htmlFuncFlags, self::ENCODING) . '</pre>';
                        // Re-set text width
                        $this->options['width'] = $pWidth;
                        // Replace content
                        $text = pq_mb_substr($text, 0, $start - $diff)
                            . $body
                            . pq_mb_substr($text, $end + pq_mb_strlen($m[0]) - $diff);

                        $diff += $len + $taglen + pq_mb_strlen($m[0]) - pq_mb_strlen($body);
                        unset($body);
                    }
                } else {
                    if ($level == 0) {
                        $start = $m[1];
                        $taglen = pq_mb_strlen($m[0]);
                    }
                    $level++;
                }
            }
    ...
    {
        switch (pq_mb_strtolower($matches[1])) {
            case 'p':
    ...
    protected function toupper($str)
    {
        ...
        $chunks = preg_split('/(<[^>]*>)/', pq_nvl($str), -1, PREG_SPLIT_NO_EMPTY | PREG_SPLIT_DELIM_CAPTURE);
    ...
    protected function strtoupper($str)
    {
        ...
        $str = pq_mb_strtoupper($str);
        $str = pq_htmlspecialchars($str, $this->htmlFuncFlags, self::ENCODING);
