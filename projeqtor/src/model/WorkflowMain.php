...
// MTY - LEAVE SYSTEM  
  ...
  public function drawSpecificItem($item, $limitToProfile=null){
    ...
    if (pq_array_key_exists('destinationWidth', $_REQUEST)) {
      $detailWidth=$_REQUEST['destinationWidth'];
	    $detailWidth=preg_replace('/[^0-9]/','', pq_nvl($detailWidth)); // only allow digits
      $detailWidth-=40;
      $detailWidth.='px';
    } else {
      $detailWidth="100%";
    ...
    if ($item=='workflowStatus') {
      ...
      if (! $print) $result .= '<div style="border: 1px solid #A0A0A0;overflow: auto; width: ' . $detailWidth . '; height:'.$height.'">';
    ...
    // WORKFLOW DIAGRAM  
      ...
      global $nbColMax;
      if (pq_array_key_exists('destinationWidth', $_REQUEST)) {
        $maxWidth=preg_replace('/[^0-9]/','', pq_nvl($detailWidth));
  ...
  public function control(){
    ...
    /*foreach ($_REQUEST as $field=>$value) { // The first status (default=recorded) is seached in the workflow definition 
      if (pq_substr($field,0,pq_strlen($search))==$search) {
      ...
      foreach ($statusList as $statColumnCode => $statColumnValue) {
        if ($statLineCode!=$statColumnCode) {
          foreach ($profileList as $profileCode => $profileValue) {
            ...
            if (pq_array_key_exists($valName,$_REQUEST)) {            
              if ($oldVal!=1) {
                $ws=new WorkflowStatus();
...
// MTY - LEAVE SYSTEM
    if (isLeavesSystemActiv()) {
        ...
        $whereLeaveTypeClause = "idLeaveType in (".pq_substr($whereLeaveTypeClause,0,-1).")";
        ...
        if ($alertLess!="" or $alertMore!="") {
            ...
            $name = pq_strtoupper(i18n("Workflow"))." - ".i18n("maintenanceOnLeavesRequired");
    ...
    public function getListStatusFromTo() {
        ...
        foreach($lstStatus as $status) {
            if (!pq_array_key_exists($status->idStatusFrom, $theStatusList)) {
                ...
                if (!pq_array_key_exists($status->idStatusTo, $theStatusList[$status->idStatusFrom])) {
                    $theStatusList[$status->idStatusFrom][$status->idStatusTo]=1;
                }
            }
        }
