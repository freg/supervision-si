...
class TenderMain extends SqlElement {
  ...
  public $_tab_4_6_smallLabel = array('untaxedAmount', 'taxPct', 'taxAmount', 'fullAmount','initial','initialLocal','discount','discountLocal', 'countTotal','countTotalLocal');
  //init
    ...
    if(RequestHandler::getBoolean('generateProjectExpenseButton')){
      ...
      if($canCreate){
        if(pq_trim(RequestHandler::getValue('objectClassName'))==get_class($this)){
          ...
          $projExpense->plannedAmountLocal = $this->totalUntaxedAmountLocal;
          $projExpense->plannedTaxAmountLocal = $this->totalTaxAmountLocal;
          $projExpense->plannedFullAmountLocal = $this->totalFullAmountLocal;
  ...
  public function copyTo($newClass, $newType, $newName, $newProject, $setOrigin, $withNotes, $withAttachments, $withLinks, $withAssignments = false, $withAffectations = false, $toProject = NULL, $toActivity = NULL, $copyToWithResult = false, $copyToWithActivityPrice=false, $copyToWithStatus = false, $copyToWithSubTask=false, $moveAfterCreate=null) {
    if ($newClass=='ProjectExpense') {
      if (! $this->totalUntaxedAmount) {
        ...
        $this->totalUntaxedAmountLocal=$this->untaxedAmountLocal;
        $this->totalFullAmountLocal=$this->fullAmountLocal;
      }
      $this->expensePlannedDate=$this->deliveryDate;
    }
    return parent::copyTo($newClass, $newType, $newName, $newProject, $setOrigin, $withNotes, $withAttachments, $withLinks, $withAssignments, $withAffectations, $toProject, $toActivity, $copyToWithResult, $copyToWithActivityPrice, $copyToWithStatus, false, $moveAfterCreate);
  }
  // ============================================================================**********
