...
class ResourceCost extends SqlElement {
  ...
  public $idle;
  //public $_sec_void;
  public static $_forceDelete=false; 
...
// ============================================================================**********
  ...
  public function save() {
    ...
    if ($this->startDate and $new) {
      ...
      $rc=new ResourceCost();
      $precs=$rc->getSqlElementsFromCriteria(null, false, $where,'id asc');
      if (count($precs)>=1) {
        self::$_forceDelete=true;
        for ($i=1; $i<count($precs); $i++) {
          $prec=$precs[$i];
          $resDel=$prec->delete();
        }
        self::$_forceDelete=false;
        $prec=$precs[0];
        $prec->endDate=addDaysToDate($this->startDate,-1);
        if ($prec->startDate==$this->startDate) $resSave=$prec->simpleDelete();
        else $resSave=$prec->simpleSave();
      }
    }
    if ($newCost) { // Cost has changed : must dispatch to stored costs
      ...
      foreach ($wkList as $wk) {
        if ($wk->idAssignment) {
          if (pq_array_key_exists($wk->idAssignment,$arrayAss)) {
      ...
      foreach ($assList as $ass) {
        if ($ass->realWork==0 and pq_trim($this->startDate)=='') {
  ...
  public function delete() { 
    $result = parent::delete();  
    if (pq_strpos($result,'lastOperationStatus" value="OK"')==0) {
      return $result;
    }
  ...
  public function deleteControl() { 
    $result='';
    if (! $this->startDate and !self::$_forceDelete) {
