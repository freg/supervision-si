...
// ============================================================================**********
  ...
  public function control() {
  	...
  	//gautier
  	if( pq_trim($this->idDocumentDirectory) != "" and pq_trim($this->idProject) != ""){
  	   $dir = new DocumentDirectory($this->idDocumentDirectory);
  	   ...
  	   if(pq_trim($dir->idProject) != "" and !in_array($this->idProject, $subProjList)){
  	     $result.="<br/>" . i18n("projectMustBeIn", array($proj->name));
  	   }
  	}
  	if(pq_trim($this->idProject) == "" and pq_trim($this->idDocumentDirectory) != ""){
  	  $dir = new DocumentDirectory($this->idDocumentDirectory);
  	  $proj = new Project($dir->idProject,true);
  	  if(pq_trim($dir->idProject) ) {
  	    $result.="<br/>" . i18n("projectMustBeIn", array($proj->name));
  	  }
  	}
  	//end
  	// Ni creation dans un repertoire clos, ni deplacement vers un repertoire clos.
  	// Le controle ne porte que sur un document neuf ou sur un changement de repertoire :
  	// un document deja range dans un repertoire clos doit rester modifiable.
  	// Pendante de SqlElement::control(), qui refuse deja la creation sous un projet
  	// ou une activite clos mais ignore idDocumentDirectory.
  	if (pq_trim($this->idDocumentDirectory) != "") {
  	  $oldDirectory=($this->id) ? $this->getOld()->idDocumentDirectory : null;
  	  if (! $this->id or $oldDirectory!=$this->idDocumentDirectory) {
  	    $dir=new DocumentDirectory($this->idDocumentDirectory);
  	    if ($dir->idle) {
  	      $result.="<br/>" . (($this->id) ? i18n("moveCancelledIdle") : i18n("errorAddOnClosedItem"))
  	              . '<br/><span style="font-size:80%">' . i18n("DocumentDirectory") . ' #' . $dir->id . '</span>';
  	    }
  	  }
  	}
  ...
  public function save() {
  	...
  	if ($old->name!=$this->name) {
  		$this->documentReference=pq_str_replace($old->name, $this->name, $this->documentReference);
  	}
  	if(!$this->id){
  	...
  	if($old->id and $old->idStatus != $this->idStatus and $this->idDocumentVersion) {
  	  $docVersion = new DocumentVersion($this->idDocumentVersion);
  	  $docVersion->idStatus=$this->idStatus;
  	  $docVersion->save();
  	}
  	//gautier 
