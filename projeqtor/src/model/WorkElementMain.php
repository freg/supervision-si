...
class WorkElementMain extends SqlElement {
    ...
    $old = $this->getOld ();
		...
		if ($this->refType) {
			...
			if(isset( $top->idSprint)){
			  $this->idSprint = $top->idSprint;
			}
		...
		if ($top and property_exists ( $top, 'idSprint' ) and ! $noDispatch) {
		  $this->idSprint = $top->idSprint;
		}
		...
		if ($top and property_exists ( $top, 'idSprint' ) and ! $noDispatch) {
		  // Check if changed Planning Activity
		  if (! pq_trim ( $old->idSprint ) and $old->idSprint != $this->idSprint) {
		    // If Activity changed, retrieve existing work
		    $crit = array (
		        'refType' => $this->refType,
		        'refId' => $this->refId
		...
		if ($old->id and $top and property_exists($top,'idSprint') and $old->idSprint and $old->idSprint!=$this->idSprint) {
		  // Update Old Activity (if changed)
		...
		if (!pq_trim($this->idSprint) and getLastOperationStatus($result)!='NO_CHANGE') {
		  // Work not counted on activity => update project
		  ProjectPlanningElement::updateSynthesis('Project',$this->idProject);
		}
		if (!$old->idSprint and $old->id and $old->idProject!=$this->idProject and getLastOperationStatus($result)!='NO_CHANGE') {
		  // Take into acocunt project change
		  ProjectPlanningElement::updateSynthesis('Project',$old->idProject);
		}
  ...
  public function simpleSave($withoutDependencies=false) {
	...
	public static function updateAssignment($work, $diff) {
	  if ($work->refType != 'Activity' and $work->refType != 'Sprint') {
			return null;
		}
		$ass = new Assignment ();
		$crit = array (
