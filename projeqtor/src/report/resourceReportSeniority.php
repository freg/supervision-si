...
if (pq_array_key_exists ( 'nbOfMonths', $_REQUEST )) {
    $paramNbOfMonths = pq_trim ( $_REQUEST ['nbOfMonths'] );
} else {
    $paramNbOfMonths = 12;
}
if ($paramNbOfMonths!="") {
...
$monthOfTheFor = $month;
$yearOfTheFor = $year;
$dayOfTheFor=lastDayOfMonth($monthOfTheFor,$yearOfTheFor);
$resource = new Resource();
$whereResource=$where." and startDate is not null and (endDate>='$yearOfTheFor-$monthOfTheFor-$dayOfTheFor' or endDate is null)";
...
for ($i=1; $i<=$paramNbOfMonths; $i++) {
    ...
    foreach ($lstResource as $res){
        $start = $res->startDate;
        $end=$res->endDate;
        if (pq_substr($start,0,7)>"$yearOfTheFor-$monthOfTheFor") continue;
        if ($end and pq_substr($end,0,7)<="$yearOfTheFor-$monthOfTheFor") continue;
