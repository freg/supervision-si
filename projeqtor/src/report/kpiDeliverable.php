...
$idOrganization="";
if (pq_array_key_exists('idOrganization',$_REQUEST) and pq_trim($_REQUEST['idOrganization'])!="") {
  $idOrganization=pq_trim($_REQUEST['idOrganization']);
  $idOrganization = Security::checkValidId($idOrganization);
}
$idProjectType="";
if (pq_array_key_exists('idProjectType',$_REQUEST) and pq_trim($_REQUEST['idProjectType'])!="") {
  $idProjectType=pq_trim($_REQUEST['idProjectType']);
  $idProjectType = Security::checkValidId($idProjectType);
}
$year="";
if (pq_array_key_exists('yearSpinner',$_REQUEST)) {
  $year=pq_trim($_REQUEST['yearSpinner']);
  $year = Security::checkValidYear($year);
}
$month="";
if (pq_array_key_exists('monthSpinner',$_REQUEST)) {
  $month=pq_trim($_REQUEST['monthSpinner']);
  $month = Security::checkValidMonth($month);
  if ($month and !$year) $year=date('Y');
}
$done=false;
if (pq_array_key_exists('onlyFinished',$_REQUEST)) {
  $done=true;
}
$showThreshold=false;
if (pq_array_key_exists('showThreshold',$_REQUEST)) {
  $showThreshold=true;
}
$scale='month';
...
$scaleHist='week';
if (pq_array_key_exists('formatHist',$_REQUEST)) {
  $scaleHist=$_REQUEST['formatHist'];
}

$class='Deliverable';
if (pq_array_key_exists('class',$_REQUEST)) {
...
if ($idProject) {
  ...
  if ($month) {
    $start=$year.'-'.$month.'-01';
    $end=$year.'-'.$month.'-'.date('t',pq_strtotime($year.'-'.$month.'-01'));
  } else if ($year) {
    $start=$year.'-01-01';
    $end=$year.'-12-31';
  }
...
if ($idProject) {
  ...
  foreach ($histList as $hist) {
    $key=$hist->refId;
    $dt=pq_substr($hist->operationDate,0,10);
  ...
  foreach ($arrayHist as $key=>$h) {
    ksort($h);
    foreach ($h as $dt=>$val) {
      if ($scaleHist=='year') $p=pq_substr($dt,0,4);
      else if ($scaleHist=='month') $p=pq_substr($dt,0,7); //$p=pq_str_replace('-','',pq_substr($dt,0,7));
      else if ($scaleHist=='week') $p=weekFormat($dt); //$p=pq_str_replace('-','',weekFormat($dt));
      else $p=$dt; //$p=pq_str_replace('-','',$dt);
      if (!isset($arrayResP[$p])) $arrayResP[$p]=array();
      $arrayResP[$p][$key]=$val;
      if ($period) {
        $pp=$dt; //$p=pq_str_replace('-','',$dt);
  ...
  foreach($arrayResP as $p=>$res) { // $arrayResP = array [period][idItem]=>idItemStatus
    ...
    else if ($scaleHist=='month') $disP=getMonthName(pq_substr($p,4),'auto').'-'.pq_substr($p,2,2);
    ...
    $lstKpiVal= $newKpiHist->getSqlElementsFromCriteria(array('idKpiDefinition'=>$kpi->id,'refType'=>'Project','refId'=>$idProject,$scaleHist=>pq_str_replace('-','',$p)),false,null,'kpiDate desc');
    $kpiDispValue='';
    $color='#ffffff';
    if (count($lstKpiVal)) {
  ...
  if ($period) {
    ...
    foreach($arrayResCurrentPeriod as $p=>$res) {
      if ($period=='year') $pp=pq_substr($p,0,4);
      else if ($period=='month') $pp=pq_str_replace('-','',pq_substr($p,0,7));
      else if ($period=='week') $pp=pq_str_replace('-','',weekFormat($p));
      else $pp=pq_str_replace('-','',$p);
    ...
    else if ($period=='month') $disP=getMonthName(pq_substr($periodValue,4),'auto').'-'.pq_substr($periodValue,2,2);
    else if ($period=='week') $disP=pq_substr($periodValue,0,4).'-'.pq_substr($periodValue,4);
  ...
  foreach($listProjects as $prj) {
    ...
    if (! pq_array_key_exists($prj->id, $visibleProjects)) continue; // Will avoid to display projects not visible to user
...
while ($date<=$end) {
  ...
  $arrDates[$date]=$date;
  if ($scale=='day') {
    $d=pq_substr($date,0,4).'-'.pq_substr($date,4,2).'-'.pq_substr($date,6,2);
    $d=addDaysToDate($d, 1);
    $date=pq_str_replace('-','',$d);
  } else if ($scale=='week') { 
    $w=pq_substr($date,4,2);
    $y=pq_substr($date,0,4);
    $w++;
...
foreach ($arrDates as $id=>$date) {
  if ($modulo!=0 and $cpt % $modulo !=0 ) {
    ...
    if ($scale=='day') {
      $arrDates[$id]=htmlFormatDate(pq_substr($date,0,4).'-'.pq_substr($date,4,2).'-'.pq_substr($date,6,2)); 
    } else if ($scale=='week') {
      $arrDates[$id]=htmlFormatDate(pq_substr($date,0,4).'-'.pq_substr($date,4,2));
    } else if ($scale=='month') {
      $arrDates[$id]=getMonthName(pq_substr($date,4),'auto').'-'.pq_substr($date,2,2);
    } else if ($scale=='year') {
      $arrDates[$id]=$date;
    }
  }
