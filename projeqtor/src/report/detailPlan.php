...
$onlyRealWork=false;
if (pq_array_key_exists('onlyRealWork',$_REQUEST)) {
  $onlyRealWork=$_REQUEST['onlyRealWork'];
};
...
if (pq_array_key_exists('idResource',$_REQUEST)) {
  $paramResource=Security::checkValidId(pq_trim($_REQUEST['idResource']));
...
foreach ($lstWork as $work) {
  ...
  if (! pq_array_key_exists($ref,$activities)) {
    $activities[$ref]=SqlList::getNameFromId($work->refType,  $work->refId);
  }
  if (! pq_array_key_exists($ref,$result[$work->idResource][$work->idProject])) {
    $result[$work->idResource][$work->idProject][$ref]=array();
    $realDays[$work->idResource][$work->idProject][$ref]=array();
  }  
  if (! pq_array_key_exists($work->day,$result[$work->idResource][$work->idProject][$ref])) {
...
if ($onlyRealWork == false) {
  ...
  foreach ($lstPlanWork as $work) {
    ...
    if (! pq_array_key_exists($work->day,$result[$work->idResource][$work->idProject][$ref]) and !isset($real[$work->idResource][$work->day])) {
      $result[$work->idResource][$work->idProject][$ref][$work->day]=0;
    }
    if (! pq_array_key_exists($work->day,$realDays[$work->idResource][$work->idProject][$ref]) and !isset($real[$work->idResource][$work->day])) { // Do not add planned if real exists 
      $result[$work->idResource][$work->idProject][$ref][$work->day]+=$work->work;
...
foreach ($resources as $idR=>$nameR) {
  ...
  if (!$paramTeam or $res->idTeam==$paramTeam) {
	  ...
	  $cpt+=1;
	  //echo '<td class="reportTableLineHeader" style="width:100px;" rowspan="'. ($cpt) . '">' . htmlEncode($nameR) . ' ('.$cpt.')'.'</td>';
	  $sortProject=array();
	  ...
	  foreach ($tmpprojects as $idP=>$proj) {
	    foreach ($result[$idR][$idP] as $idA=>$acti) { 
		    if (pq_array_key_exists($idP, $projects)) {
		      $cptRes++;
		      echo '<td class="reportTableLineHeader" style="width:100px;border-bottom:0;border-top:0" >' . (($cptRes==1)?htmlEncode($nameR):'') .'</td>';
		      ...
		      for ($i=1; $i<=$nbDays;$i++) {
		        ...
		        if ($days[$day]=="off") {
		          ...
		          if (! pq_array_key_exists($day, $realDays[$idR][$idP][$idA]) 
		          and pq_array_key_exists($day,$result[$idR][$idP][$idA])) {
		            $style=$plannedStyle;
		            $ital=true;
		          }
		        }
		        echo '<td class="reportTableData" ' . $style . ' valign="top">';
		        if (pq_array_key_exists($day,$result[$idR][$idP][$idA])) {
		          echo ($ital)?'<i>':'';
	  ...
	  echo '<td class="reportTableLineHeader" colspan="3">' . i18n('sum') . '</td>';
	  $lineSum=0;
	  for ($i=1; $i<=$nbDays;$i++) {
	    $style='';
	    $day=$startDate+$i-1;
