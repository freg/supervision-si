...
if (! isset ( $includedReport )) {
  ...
  $paramUrgency = array();
  if (pq_array_key_exists ( 'urgency', $_REQUEST )) {
  ...
  if (pq_array_key_exists ( 'criticality', $_REQUEST )) {
...
if ($nbDay != 0) {
  for($i = 1; $i <= $nbDay; $i ++) {
    foreach ( $lstReqNew as $t ) {
      if ($t->doneDate != '') {
        $startReq = pq_strtotime ( $t->doneDate );
        if ($startReq < (pq_strtotime ( $startDate ) + ($i * 24 * 60 * 60)) and $t->doneDate != '') {
          $nbReq = $nbReq - 1;
          $t->doneDate = '';
        }
      }
