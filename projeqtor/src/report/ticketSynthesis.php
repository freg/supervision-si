...
if (!empty($paramPriorities) and $paramPriorities[0] != 'undefined') {
	...
	$where = pq_substr($where, 0, -2); //To remove the last comma and space
	$where.=")";
...
foreach ($lstTicket as $t) {
  $urgency=($t->idUrgency==null or pq_trim($t->idUrgency)=='')?'0':$t->idUrgency;
  $criticality=($t->idCriticality==null or pq_trim($t->idCriticality)=='')?'0':$t->idCriticality;
  $priority=($t->idPriority==null or pq_trim($t->idPriority)=='')?'0':$t->idPriority;
  $type=$t->idTicketType;
  $issuer=$t->idUser;
  $responsible=($t->idResource==null or pq_trim($t->idResource)=='')?'0':$t->idResource;
  //urgency
