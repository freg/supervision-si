...
if (pq_array_key_exists('idResource',$_REQUEST)) {
  ...
  $paramResource = preg_replace('/[^0-9]/', '', pq_nvl($paramResource));
  ...
  if (!$canChangeResource and $paramResource!=$user->id and empty($cronnedScript)) {
...
foreach ($lstWork as $work) {
  ...
  if (!$paramActivityType or (isset($act) and ($paramActivityType == $act->idActivityType))) {
    ...
    if ($work->refType == 'Activity' and !pq_array_key_exists($work->idResource, $activities)) {
      $activities[$work->idResource] = array();
    }
    if ($work->refType == 'Activity' and !pq_array_key_exists($work->refId, $activities[$work->idResource])) {
    ...
    if (!pq_array_key_exists($work->refId, $activityWork)) {
      $activityWork[$work->refId] = array();
    }
    if (!pq_array_key_exists($work->idProject, $activityWork[$work->refId])) {
...
foreach ($resources as $idR=>$nameR) {
  if ($paramTeam) {
    ...
    if($res->idTeam!=$paramTeam)continue;
  }
...
foreach ($projects as $id=>$name) {
  if (isset($arrayCodes[$id])) {
    echo '<td style="width:'.$colWidth.'%" class="reportTableColumnHeader" '.excelFormatCell('subheader',20).'>' . htmlEncode($arrayCodes[$id]) . '</td>';
  }
}
echo '</tr>';
}
...
foreach ($resources as $idR=>$nameR) {
  ...
  //if (!$paramTeam or $res->idTeam==$paramTeam) {
		$sumRes=0;
		if(pq_array_key_exists($idR, $activities))$rowspan = count($activities[$idR]);
		else continue ;
	  ...
	  foreach ($activityRes[$idR] as $idAct=>$proj){
        $count++;$nameA="";
        if (pq_array_key_exists($idR, $activities)) {
         	if (pq_array_key_exists($idAct, $activities[$idR])) {
       ...
       if($count==1){
    	    ...
    	    drawLine($projects,$activityRes, $idR, $idAct, $colWidth,$sumRes,$sum,$sumProj);
    	    echo '</tr>';
          }else{
            echo '<tr>';
            echo '<td style="width:10%" class="reportTableLineHeader" '.excelFormatCell('rowheader').'>' . htmlEncode($nameA) . '</td>';
...
function drawLine ($projects,$activityRes,$idR,$idAct,$colWidth,$sumRes,&$sum,$sumProj){
	     foreach ($projects as $idP=>$nameP){
	      $idExplo=pq_explode('-',$idP);
	      $idP=$idExplo[1];
        	if (pq_array_key_exists($idR, $activityRes)) {
        		if (pq_array_key_exists($idAct, $activityRes[$idR])) {
        		  if(pq_array_key_exists($idP, $activityRes[$idR][$idAct])){
