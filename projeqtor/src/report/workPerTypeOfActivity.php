...
//#407
$paramActivityType='';
if (pq_array_key_exists('idActivityType', $_REQUEST)) {
  $paramActivityType=pq_trim($_REQUEST['idActivityType']);
...
$showDetail=false;
if (pq_array_key_exists('showDetail',$_REQUEST)) {
  $showDetail=true;
}
...
$paramResource='';
if (pq_array_key_exists('idResource',$_REQUEST)) {
...
foreach ($lstWork as $work) {
  if ($paramActivityType and $work->refType=='Activity' ) {
    if (pq_array_key_exists($work->refId, $alreadyExistingActivity)) {
      $act = $alreadyExistingActivity[$work->refId];
      $actType = $alreadyExistingType[$work->refId];
    } else {
      $act = new Activity($work->refId);
      $actType = new Type($act->idActivityType);
      $alreadyExistingActivity[$work->refId] = $act;
      $alreadyExistingType[$work->refId] = $actType;
    }
  }else{
    $act = new $work->refType($work->refId);
    $typeName=$act->getObjectTypeName();
    $actType = new Type($act->$typeName);
  }
  if ($paramActivityType == "" or  (isset($act) and $paramActivityType == $act->idActivityType and $work->refType=='Activity')) {
    ...
    if (! pq_array_key_exists($work->idResource,$result)) {
      $result[$work->idResource]=array();
    }
    if (! pq_array_key_exists($work->idProject,$result[$work->idResource])) {
      $result[$work->idResource][$work->idProject]=0;
    }
    // #407
    // #ActNames;
    if (!pq_array_key_exists($work->idResource, $actNames)) {
    ...
    else if (!pq_array_key_exists($actType->name, $actNames[$work->idResource])) {
      $actNames[$work->idResource][$actType->name] = array();
      $actNames[$work->idResource][$actType->name][$act->name] = $work->work + 0;
    }
    else if (!pq_array_key_exists($act->name, $actNames[$work->idResource][$actType->name])) {
      $actNames[$work->idResource][$actType->name][$act->name] = $work->work + 0;
    }
    else
      $actNames[$work->idResource][$actType->name][$act->name] += $work->work;
    // #workActivity
    if (!pq_array_key_exists($work->idResource, $workActivity)) {
    ...
    else if (!pq_array_key_exists($work->idProject, $workActivity[$work->idResource])) {
      $workActivity[$work->idResource][$work->idProject] = array();
      $workActivity[$work->idResource][$work->idProject][$actType->name] = array();
      $workActivity[$work->idResource][$work->idProject][$actType->name][$act->name] = $work->work + 0;
    }
    else if (!pq_array_key_exists($actType->name, $workActivity[$work->idResource][$work->idProject])) {
      $workActivity[$work->idResource][$work->idProject][$actType->name] = array();
      $workActivity[$work->idResource][$work->idProject][$actType->name][$act->name] = $work->work + 0;
    }
    else if (!pq_array_key_exists($act->name, $workActivity[$work->idResource][$work->idProject][$actType->name]))
      $workActivity[$work->idResource][$work->idProject][$actType->name][$act->name] = $work->work + 0;
    else
      $workActivity[$work->idResource][$work->idProject][$actType->name][$act->name] += $work->work;
    // #sumWorkAct
    if (!pq_array_key_exists($work->idResource, $sumWorkAct)) {
      $sumWorkAct[$work->idResource] = array();
      $sumWorkAct[$work->idResource][$actType->name] = $work->work + 0;
    }
    else if (!pq_array_key_exists($actType->name, $sumWorkAct[$work->idResource])) {
      $sumWorkAct[$work->idResource][$actType->name] = $work->work + 0;
    }
    else
      $sumWorkAct[$work->idResource][$actType->name] += $work->work;
    // #ActTypes
    if (!pq_array_key_exists($work->idResource, $resActTypes)) {
...
foreach ($resources as $idR=>$nameR) {
  ...
  if (!$paramTeam or $res->idTeam==$paramTeam) {
    foreach ($projects as $idP=>$nameP) {
      ...
      $idP=$idExplo[1];
      if (pq_array_key_exists($idR, $result)) {
        if (pq_array_key_exists($idP, $result[$idR])) {
          $val=$result[$idR][$idP];
          $sumProj[$idP]+=$val;
        }
      }
...
foreach ($projects as $id=>$name) {
  ...
  $id=$idExplo[1];
  if($sumProj[$id] != 0)
  echo '<td style="width:'. (pq_array_key_exists('idActivityType', $_REQUEST) ? ($colWidth * 3 / 4) : $colWidth) .'%" class="reportTableColumnHeader">' . htmlEncode($name) . '</td>';
}
echo '</tr>';
$sum=0;
$sumAct = 0;
...
foreach ($resources as $idR=>$nameR) {
  ...
  if (!$paramTeam or $res->idTeam==$paramTeam) {
    ...
    } else if (pq_array_key_exists('idActivityType', $_REQUEST))
    ...
    for ($nbActTypes = 0; $nbActTypes < count($resActTypes[$idR]); $nbActTypes++) {
      ...
      if (!$showDetail) {
        ...
        foreach ($projects as $idP=>$nameP) {
          ...
          $idP=$idExplo[1];
          if($sumProj[$idP] != 0){
            echo '<td style="width:' . (pq_array_key_exists('idActivityType', $_REQUEST) ? ($colWidth / 3) : $colWidth) . '%" class="reportTableData">';
            if (pq_array_key_exists($idR, $workActivity)) {
              if (pq_array_key_exists($idP, $workActivity[$idR])) {
                if (pq_array_key_exists($resActTypes[$idR][$nbActTypes], $workActivity[$idR][$idP])) {
          ...
          foreach ($projects as $idP=>$nameP) {
            ...
            if($sumProj[$idP] != 0){
              ...
              if (pq_array_key_exists($idR, $workActivity)) {
                if (pq_array_key_exists($idP, $workActivity[$idR])) {
                  if (pq_array_key_exists($resActTypes[$idR][$nbActTypes], $workActivity[$idR][$idP])) {
                    if (pq_array_key_exists($acts, $workActivity[$idR][$idP][$resActTypes[$idR][$nbActTypes]])) {
...
if ($showDetail)
  echo '<td class="reportTableColumnHeader">' . Work::displayWorkWithUnit($sum).'</td>';
