...
foreach ($lstWork as $work) {
  ...
  if (! pq_array_key_exists($work->idProject,$realDays[$work->idResource])) {
    $realDays[$work->idResource][$work->idProject]=array();
  }
  if (! pq_array_key_exists($work->day,$result[$work->idResource][$work->idProject])) {
  ...
  if (! pq_array_key_exists($work->day,$workDayResource[$work->idResource])) {
...
foreach ($lstPlanWork as $work) {
  ...
  if (! pq_array_key_exists($work->day,$result[$work->idResource][$work->idProject]) and !isset($real[$work->idResource][$work->day])) {
    $result[$work->idResource][$work->idProject][$work->day]=0;
  }
  if (! pq_array_key_exists($work->day,$workDayResource[$work->idResource]) and !isset($real[$work->idResource][$work->day])) {
    $workDayResource[$work->idResource][$work->day]=0;
  }
  if (! pq_array_key_exists($work->day,$realDays[$work->idResource][$work->idProject])  and !isset($real[$work->idResource][$work->day])) { // Do not add planned if real exists 
    $result[$work->idResource][$work->idProject][$work->day]+=$work->work;
    $workDayResource[$work->idResource][$work->day]+=$work->work;
	    ...
	    if (pq_array_key_exists($idP, $projects)) {
	      ...
	      for ($i=1; $i<=$nbDays;$i++) {
	        ...
	        if ($days[$day]=="off") {
	          ...
	          and pq_array_key_exists($day,$result[$idR][$idP])) {
	            $style=$plannedStyle;
	            $ital=true;
	          }
	        }	    
	        echo '<td class="reportTableData" ' . $style . ' valign="top">';
	        if (pq_array_key_exists($day,$result[$idR][$idP])) {
	          echo ($ital)?'<i>':'';
