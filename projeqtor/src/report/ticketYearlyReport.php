...
foreach ($lstTicket as $t) {
  //CHANGE qCazelles - Report fiscal year - Ticket #128
  if (pq_substr($t->creationDateTime,0,4)==$paramYear) {
    $month=intval(pq_substr($t->creationDateTime,5,2));
    if ($month>=$paramMonth) {
  ...
  //ADD qCazelles
  else if (pq_substr($t->creationDateTime,0,4)==$endYear) {
    $month=intval(pq_substr($t->creationDateTime,5,2));
    if ($month<=$paramMonth) {
  ...
  //END ADD qCazelles
  if (pq_substr($t->doneDateTime,0,4)==$paramYear) {
    $month=intval(pq_substr($t->doneDateTime,5,2));
    if ($month>=$paramMonth) {
  ...
  //Old
  //else if (pq_substr($t->doneDateTime,0,4)==$paramYear) {
  //New
  else if (pq_substr($t->doneDateTime,0,4)==$endYear) {
  ...
  //END ADD qCazelles
  if (pq_substr($t->idleDateTime,0,4)==$paramYear) {
    $month=intval(pq_substr($t->idleDateTime,5,2));
    if ($month>=$paramMonth) {
  ...
  //ADD qCazellles
  else if (pq_substr($t->idleDateTime,0,4)==$endYear) {
    $month=intval(pq_substr($t->idleDateTime,5,2));
