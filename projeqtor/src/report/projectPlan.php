...
foreach ($lstWork as $work) {
  ...
  if (! pq_array_key_exists($work->idResource,$result[$work->idProject])) {
    $result[$work->idProject][$work->idResource]=array();
    $realDays[$work->idProject][$work->idResource]=array();
  }
  if (! pq_array_key_exists($work->day,$result[$work->idProject][$work->idResource])) {
...
foreach ($lstPlanWork as $work) {
  ...
  if (! pq_array_key_exists($work->day,$result[$work->idProject][$work->idResource]) and !isset($real[$work->idResource][$work->day])) {
    $result[$work->idProject][$work->idResource][$work->day]=0;
  }
  if (! pq_array_key_exists($work->day,$realDays[$work->idProject][$work->idResource]) and !isset($real[$work->idResource][$work->day])) { // Do not add planned if real exists 
    $result[$work->idProject][$work->idResource][$work->day]+=$work->work;
    ...
    if (pq_array_key_exists($idR, $resources)) {
      ...
      for ($i=1; $i<=$nbDays;$i++) {
        ...
        if ($days[$day]=="off") {
          ...
          if (! pq_array_key_exists($day, $realDays[$idP][$idR]) 
          and pq_array_key_exists($day,$result[$idP][$idR])) {
            $style=$plannedStyle;
            $ital=true;
          }
        }
        echo '<td class="reportTableData" ' . $style . ' valign="top">';
        if (pq_array_key_exists($day,$result[$idP][$idR])) {
          echo ($ital)?'<i>':'';
