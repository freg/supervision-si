...
foreach ($lstResource as $res){
    if (pq_substr($res->startDate,0,4)==$year) {
        $paramMonth=intval(pq_substr($res->startDate,5,2));
        if ($paramMonth>=$month) {
            $input[$paramMonth - ($month - 1)]+=1;
            $input[13]+=1;
        }
    } else if (pq_substr($res->startDate,0,4)==$endYear) {
        $paramMonth=intval(pq_substr($res->startDate,5,2));
        if ($paramMonth<=$month) {
    ...
    if (pq_substr($res->endDate,0,4)==$year) {
        $paramMonth=intval(pq_substr($res->endDate,5,2));
        if ($paramMonth>=$month) {
            $output[$paramMonth - ($month - 1)]+=1;
            $output[13]+=1;
        }
    } else if (pq_substr($res->endDate,0,4)==$endYear) {
        $paramMonth=intval(pq_substr($res->endDate,5,2));
        if ($paramMonth<=$month) {
