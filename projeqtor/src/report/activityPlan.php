...
foreach ($lstWork as $work) {
  ...
  if (! pq_array_key_exists($ref,$result[$work->idProject])) {
  ...
  if (! pq_array_key_exists($work->idResource,$result[$work->idProject][$ref])) {
    $result[$work->idProject][$ref][$work->idResource]=array();
    $realDays[$work->idProject][$ref][$work->idResource]=array();
  }  
  if (! pq_array_key_exists($work->day,$result[$work->idProject][$ref][$work->idResource])) {
...
if ($onlyRealWork == false) {
  ...
  foreach ($lstPlanWork as $work) {
    ...
    if (! pq_array_key_exists($work->day,$result[$work->idProject][$ref][$work->idResource]) and !isset($real[$work->idResource][$work->day])) {
      $result[$work->idProject][$ref][$work->idResource][$work->day]=0;
    }
    if (! pq_array_key_exists($work->day,$realDays[$work->idProject][$ref][$work->idResource]) and !isset($real[$work->idResource][$work->day])) { // Do not add planned if real exists 
    	 $result[$work->idProject][$ref][$work->idResource][$work->day]+=$work->work;
...
foreach ($projects as $idP=>$nameP) {
  ...
  foreach ($result[$idP] as $idA=>$acti) {
    foreach ($result[$idP][$idA] as $idR=>$ress) {
      $sumNpw=0;
	    if (pq_array_key_exists($idA, $activities)) {
	      ...
	      for ($i=1; $i<=$nbDays;$i++) {
	        ...
	        if ($days[$day]=="off") {
	          ...
	          if (  ! pq_array_key_exists($day, $realDays[$idP][$idA][$idR])  
	          and pq_array_key_exists($day,$result[$idP][$idA][$idR])) {
	            $style=$plannedStyle;
	            $ital=true;
	          }
	        }
	        echo '<td class="reportTableData" ' . $style . ' valign="top">';
	        if (pq_array_key_exists($day,$result[$idP][$idA][$idR])) {
	            echo ($ital)?'<i>':'';
