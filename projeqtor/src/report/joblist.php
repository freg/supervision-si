...
$paramActivity = '';
$paramProject = '';
if (pq_array_key_exists('idActivity', $_REQUEST)) {
    $paramActivity = pq_trim($_REQUEST['idActivity']);
}
...
foreach ($jobDefinitions as $jobDef) {
    if (!pq_array_key_exists($jobDef->id, $aDefLines)) {
    ...
    if (!pq_array_key_exists($jobDef->idJoblistDefinition, $aDefLinesCount)) {
...
foreach ($lstActivity as $activity) {
    ...
    if (pq_array_key_exists($activity->id, $result)) {
        $iCount = 0;
            ...
            if ($outMode == 'csv') {
                if (isset($result[$activity->id][$sKey])) {
                    if ($result[$activity->id][$sKey]->value) {
                        ...
                        echo htmlFormatDate(pq_substr($result[$activity->id][$sKey]->creationDate, 0, 10));
                    }
                    echo ';';
                } else {
                    echo ';';
                ...
                if (isset($result[$activity->id][$sKey])) {
                    ...
                    if (!is_null($result[$activity->id][$sKey]->creationDate)) {
                        $title .= 'Due for ' . htmlFormatDate(pq_substr($result[$activity->id][$sKey]->creationDate, 0, 10)) . "\n";
                    }
        ...
        $pct=(count($aDefLines))?round((($iCount / count($aDefLines)) * 100), 2):'';
        if ($outMode == 'csv') {
            echo  $pct . ';';
        } else {
            echo '<td class="reportTableLineHeader" style="text-align: right">' . $pct . ' %</td>';
        }
        $totalChecked += $iCount;
