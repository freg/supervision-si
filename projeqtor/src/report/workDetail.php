...
$paramNbMonth='';
if (pq_array_key_exists('NbMonthSpinner',$_REQUEST)) {
...
foreach ($lstWork as $work) {
  ...
  if ($paramActivityType == "" or ($paramActivityType == $actTypeId and $work->refType=='Activity')) {
    ...
    $refId = $work->refId;
    $key = $refType . "#" . $refId;
    if (!pq_array_key_exists($key, $activities)) {
      if ($refType) {
    ...
    if (!pq_array_key_exists($key, $result[$work->idResource])) {
  ...
  if (!$paramTeam or $res->idTeam==$paramTeam) {
	  ...
	  foreach ($activities as $key=>$nameA) {
	    if (pq_array_key_exists($idR, $result)) {
	      if (pq_array_key_exists($key, $result[$idR])) {
	        $val=$result[$idR][$key];
	        $sumRes+=$val; 
	        $sum+=$val;
