...
if (pq_array_key_exists('showDoneVersions',$_REQUEST)) {
  $paramDoneVersion=true;
};
$paramOtherVersion=false;
  if (pq_array_key_exists('otherVersions',$_REQUEST)) {
...
foreach ($lstVersion as $versId=>$versName) {
  ...
  //if ($versDate=='')
  if ($version->realEisDate) {
    $date = '('.htmlFormatDate($version->realEisDate).')';
  } else if ($version->plannedEisDate) {
    $date = '('.htmlFormatDate($version->plannedEisDate).')';
  } else if ($version->initialEisDate) {
    $date = '('.htmlFormatDate($version->initialEisDate).')';
  } else {
    $date='';
  }
  echo '<td class="reportTableHeader" style="width:40%" colspan="3">' . htmlEncode($version->name) . ' ' . $date. '</td>';
  ...
  echo '<td class="largeReportHeader" style="width:5%;text-align:center;">' . pq_strtolower(i18n('sum')) . '</td>';
  echo '</tr>';
  $sumInitial=0;
  $sumReal=0;
  $sumLeft=0;
  ...
  $cpt=0;
    ...
    foreach ($lst as $item) {
      ...
      $canSum=true;
      if ($class=='Ticket') {
        if ($item->idActivity and $item->idTargetProductVersion==$versId) {
          $versAct=SqlList::getFieldFromId('Activity', $item->idActivity, 'idTargetProductVersion');
          if ($versAct==$versId) $canSum=false;
        }
      }
      if ($canSum) {
        $sumInitial+=$initial;
