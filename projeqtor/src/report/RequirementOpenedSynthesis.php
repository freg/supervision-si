...
if (! isset($includedReport)) {
  ...
  include_once("../external/pChart2/class/pPie.class.php");
  
  // set Parameters
  $paramProject=pq_trim(RequestHandler::getId('idProject',false));
  $paramYear=pq_trim(RequestHandler::getYear('yearSpinner'));
  $paramMonth=pq_trim(RequestHandler::getMonth('monthSpinner'));
//   $paramWeek=pq_trim(RequestHandler::getDatetime('weekSpinner'));
  $paramResponsible=pq_trim(RequestHandler::getId('responsible',false));
  $paramRequirementType=pq_trim(RequestHandler::getId('idRequirementType',false));
  $paramRequestor=pq_trim(RequestHandler::getId('requestor',false));
  $paramIssuer=pq_trim(RequestHandler::getId('issuer',false));
  ...
  if (pq_array_key_exists('priorities',$_REQUEST)) {
  ...
  $periodType="";
  $periodValue="";
  if (pq_array_key_exists('periodType',$_REQUEST)) {
  ...
  if (!empty($paramPriorities)) {
  	...
  	$headerParameters=pq_substr($headerParameters, 0, -2);
...
foreach ($lstRequirement as $r) {
  $urgency=($r->idUrgency==null or pq_trim($r->idUrgency)=='')?'0':$r->idUrgency;
  $criticality=($r->idCriticality==null or pq_trim($r->idCriticality)=='')?'0':$r->idCriticality;
  $priority=($r->idPriority==null or pq_trim($r->idPriority)=='')?'0':$r->idPriority;
  $type=$r->idRequirementType;
  $issuer=$r->idUser;
  $responsible=($r->idResource==null or pq_trim($r->idResource)=='')?'0':$r->idResource;
  //urgency
  if (! pq_array_key_exists($urgency, $lstUrgency)) {
    $lstUrgency[$urgency]=0;
  }
  $lstUrgency[$urgency]+=1;
  //criticality
  if (! pq_array_key_exists($criticality, $lstCriticality)) {
    $lstCriticality[$criticality]=0;
  }
  $lstCriticality[$criticality]+=1;
  //priority
  if (! pq_array_key_exists($priority, $lstPriority)) {
    $lstPriority[$priority]=0;
  }
  $lstPriority[$priority]+=1;
  //type
  if (! pq_array_key_exists($type, $lstType)) {
    $lstType[$type]=0;
  }
  $lstType[$type]+=1;
  //issuer
  if (! pq_array_key_exists($issuer, $lstIssuer)) {
    $lstIssuer[$issuer]=0;
  }
  $lstIssuer[$issuer]+=1;
  //responsible
  if (! pq_array_key_exists($responsible, $lstResponsible)) {
...
function drawSynthesisTable($scope, $lst) {
  ...
  foreach ($lstRef as $code=>$val) {
    if (pq_array_key_exists($code, $lst)) {
      echo '<tr><td class="reportTableHeader" style="width:150px">';
      echo $val;
