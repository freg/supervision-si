...
foreach ($lstPlanWork as $work) {
  ...
  //if (! pq_array_key_exists($work->day,$real)) { // Do not add planned if real exists 
    $result[$work->idResource][$work->day]+=$work->work;
  //}
}
...
//    $maxCapa=$capacity[$idR];
    ...
    for ($i=1; $i<=$nbDays;$i++) {
      $day=$startDate+$i-1;
      $dayDate=pq_substr($day,0,4) . "-" . pq_substr($day,4,2) . "-" . pq_substr($day,6,2);
  ...
  if (!$paramTeam or $res->idTeam==$paramTeam) {
	  ...
	  for ($i=1; $i<=$nbDays;$i++) {
	    ...
	    $italic=false;
	    //if ($days[$day]=="off") {
	    if (isOffDay(pq_substr($day,0,4) . "-" . pq_substr($day,4,2) . "-" . pq_substr($day,6,2), $resourceCalendar[$idR])) {	
	      $style=$weekendStyle;
	    } else {
	      ...
	      //if (! pq_array_key_exists($day,$real) and pq_array_key_exists($day,$result[$idR])) {
	      if (pq_array_key_exists($idR,$real) and ! pq_array_key_exists($day,$real[$idR]) and pq_array_key_exists($day,$result[$idR])) {
