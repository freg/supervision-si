...
$idResource="";
if (pq_array_key_exists('idResource',$_REQUEST) and pq_trim($_REQUEST['idResource'])!="") {
...
$scale='month';
if (pq_array_key_exists('scale',$_REQUEST)) {
...
if($showAdminProj =='on'){
  $queryWhere= "( (".$queryWhere.") or t1.idProject in " . Project::getAdminitrativeProjectList() . ")";
} else if ($idProject) {
  $queryWhere= "( (".$queryWhere.") or t1.idProject = " . $idProject . ")";
} else {
...
for ($i=1;$i<=2;$i++) {
  ...
  while ($line = Sql::fetchLine($result)) {
    ...
    $work=$line['sumwork'];
    if (! pq_array_key_exists($proj, $tab) ) {
...
while ($date<=$end) {
  ...
  if (! pq_array_key_exists($year,$arrYear)) {
    $arrYear[$year]=0;
  }
  $arrYear[$year]+=1;
  if ($scale=='week') {
...
foreach($tabSorted as $proj=>$lists) {
  $sumProj[$proj]=array();
  ...
  for ($i=1; $i<=2; $i++) {
    ...
    foreach($arrDates as $date) {
      ...
      $val=0;
      if (pq_array_key_exists($mode, $lists) and pq_array_key_exists($date,$lists[$mode])) {
        $val=$lists[$mode][$date];
      }
...
foreach($sumProjUnit as $id=>$vals) {
  ...
  $dataSet->addPoints($vals,$id.$tab[$id]['name']);
  $dataSet->setSerieDescription($id.$tab[$id]['name'],$tab[$id]['name']);
  $dataSet->setSerieOnAxis($id.$tab[$id]['name'],0);
...
foreach($sumProjUnit as $id=>$vals) {
  $serie+=1;
  $dataSet->removeSerie($id.$tab[$id]['name']);
}
