...
$type="";
if (pq_array_key_exists('idMilestoneType',$_REQUEST)) {
  $type=pq_trim($_REQUEST['idMilestoneType']);
};
$startDateReport="";
...
$showToday=false;
if (pq_array_key_exists('showBurndownToday',$_REQUEST)) {
  $showToday=true;
}
$legend='included';
if (pq_array_key_exists('showBurndownLegendOnTop',$_REQUEST)) {
  $legend="top";
}
...
while ($line = Sql::fetchLine($resultMile)) {
  ...
  $arrayMile[$idpe]['dates'][$mpeEndDate]=pq_strtotime($mpeEndDate.$refTime);
...
if (!$showIdle) {
  ...
  $query=$querySelect.$queryFrom.$queryWhere;
  $query.=' UNION '.$querySelect.pq_str_replace($hTable,$haTable,$queryFrom).$queryWhere.$queryOrder;
}
...
while ($line = Sql::fetchLine($resultPlanned)) {
  ...
  if (!$arrayMile[$idpe]['real'] or $day<$arrayMile[$idpe]['lastDate']) { // #2590
    $arrayMile[$idpe]['dates'][$day]=pq_strtotime($new.$refTime);
...
//$start=pq_substr($start,0,8).'01';
//$end=pq_substr($end,0,8).date('t',pq_strtotime($end));
$date=$start;
$arrDates=array();
ksort($existingDates);
$arrayShow=array();
while ($date<=$end) {
  ...
  if ($scale=='day') { 
    $arrDates[$date]=pq_strtotime($date.$refTime);
    if ($show) $arrayShow[$date.$refTime]=true;
  } else {
    $dt=new DateTime();
    $dt->setTimestamp(pq_strtotime($date.$refTime));
...
foreach ($arrLabel as $idx=>$val) {
  if ($scale=='day') {
    $arrLabel[$idx]=pq_strtotime($val.$refTime);
  }
  if ($scale=='month') {
    //$arrLabel[$idx]=pq_strtotime($val.$refTime);
  }
