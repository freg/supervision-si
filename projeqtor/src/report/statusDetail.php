...
if (! isset($includedReport)) {
  ...
  if (pq_array_key_exists('refType',$_REQUEST)) {
    $paramRefType=pq_trim($_REQUEST['refType']);
...
while ($line = Sql::fetchLine($result)) {
  ...
  $val=$line['nb'];
  if (! pq_array_key_exists($type, $arr)) {
    $arr[$type]=array();
  }
  if (! pq_array_key_exists($status, $arrStatus)) {
    $arrStatus[$status]=0;
  }
...
foreach ($lstStatus as $id=>$st) {
  if (! pq_array_key_exists($id, $arrStatus)) {
    unset($lstStatus[$id]);
  }
}
$lstType=SqlList::getList($refType . 'Type');
foreach ($lstType as $id=>$st) {
  if (! pq_array_key_exists($id, $arr)) {
    unset($lstType[$id]);
  }
...
if (count($lstStatus)>0) {
	...
	foreach($arr as $id=>$arrType) {
	  ...
	  foreach ($lstStatus as $is=>$status) {
	    if (pq_array_key_exists($is,$arrType)) {
	      $temp[$is]=$arrType[$is];
	    } else {
	      $temp[$is]=0;
	    }
