...
$paramProduct=pq_trim(RequestHandler::getId('idProduct',false));
$paramVersion=pq_trim(RequestHandler::getId('idVersion',false));
...
foreach ($lstRequirement as $r) {
	if (pq_substr($r->creationDateTime,0,4)==$paramYear) {
		$month=intval(pq_substr($r->creationDateTime,5,2));
		if ($month>=$paramMonth) {
			$created[$month - ($paramMonth - 1)]+=1;
			$created[13]+=1;
		}
	}
	else if (pq_substr($r->creationDateTime,0,4)==$endYear) {
		$month=intval(pq_substr($r->creationDateTime,5,2));
		if ($month<=$paramMonth) {
	...
	if (pq_substr($r->doneDate,0,4)==$paramYear) {
		$month=intval(pq_substr($r->doneDate,5,2));
		if ($month>=$paramMonth) {
			$done[$month - ($paramMonth - 1)]+=1;
			$done[13]+=1;
		}
	}
	else if (pq_substr($r->doneDate,0,4)==$endYear) {
		$month=intval(pq_substr($r->doneDate,5,2));
		$done[12 - $paramMonth + $month + 1]+=1;
		$done[13]+=1;
	}
	if (pq_substr($r->idleDate,0,4)==$paramYear) {
		$month=intval(pq_substr($r->idleDate,5,2));
		if ($month>=$paramMonth) {
			$closed[$month - ($paramMonth - 1)]+=1;
			$closed[13]+=1;
		}
	}
	else if (pq_substr($r->idleDate,0,4)==$endYear) {
		$month=intval(pq_substr($r->idleDate,5,2));
