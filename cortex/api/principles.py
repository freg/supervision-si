...
PRINCIPLES = {
    ...
    # -- alertes (étape 5, #466) --------------------------------------------
    "policy-role-place": {"title": "Un routeur de site vaut plus qu'un poste", "base": 0.9, "scope": "alerte",
                          "statement": "La politique d'alerte se choisit par rôle de la cause, site, type d'entité et sévérité minimale ; la première politique qui correspond décide de la priorité, des canaux et de l'escalade.",
                          "limit": "un rôle mal déduit (rôle = hypothèse) applique la mauvaise politique : la raison est affichée sur chaque décision."},
    "notify-per-incident": {"title": "Une notification par incident, jamais par événement", "base": 1.0, "scope": "alerte",
                            "statement": "C'est l'incident (cause + entités regroupées) qui est notifié, une fois à l'ouverture, une fois en escalade, une fois à la résolution si la politique le veut.",
                            "limit": "un incident mal regroupé (fenêtre trop large) noie deux pannes dans une notification."},
    "escalation": {"title": "Un incident non acquitté s'escalade", "base": 0.9, "scope": "alerte",
                   "statement": "Passé le délai de sa politique sans accusé, l'incident est renotifié une seule fois sur les canaux d'escalade (SMS par défaut).",
                   "limit": "l'accusé d'un événement source (tuile d'origine) n'est pas vu : acquitter dans Cortex."},
    "silence-maintenance": {"title": "Un silence planifié n'efface pas, il tait", "base": 0.9, "scope": "alerte",
                            "statement": "Pendant une maintenance déclarée (site, entités ou rôles, plage horaire, ticket), les incidents couverts restent visibles et comptés mais ne sont pas notifiés.",
                            "limit": "un silence trop large (site entier) cache une vraie panne : la raison « silence » est tracée sur chaque incident tu."},
    # -- causalité -------------------------------------------------------
