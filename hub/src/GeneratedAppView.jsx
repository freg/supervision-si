...
// #445 : `externalSpec` (projection de l'outil unique sur une application)
// remplace la spec enregistrée -- rendu identique, pas de régénération ni
// d'édition (la spec unique se corrige dans celles des applications).
export default function GeneratedAppView({ retroApiBase, dbaApiBase, app, onClose, externalSpec, heading }) {
  ...
  const readOnly = !!externalSpec;
  const load = useCallback(async (regenerate) => {
    setBusy(true); setError(null);
    const r = externalSpec || await fetchUiSpec(retroApiBase, app, regenerate);
    setBusy(false);
  ...
  return (
    <div className="hub-card hub-settings-section" style={{ borderColor: "var(--accent, #2b5797)" }}>
      <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
        <h2 style={{ margin: 0 }}>{heading || `Application générée — « ${app} »`}</h2>
        ...
        {!readOnly && <button className="secondary" disabled={busy} onClick={() => load(true)} title="recalculer depuis les parcours, la carte et les colonnes réelles (choix manuels conservés)">↻ Régénérer</button>}
        {onClose && <button className="secondary" onClick={onClose}>Fermer</button>}
      </div>
              ...
              <>
                ...
                {record && (
                  <form onSubmit={saveRecord} className="hub-card" style={{ margin: 0 }}>
                    ...
                    {missingFields(record.screen).length > 0 && <p className="muted" style={{ fontSize: 12 }}>Sans colonne dans cette application (non saisissables) : {missingFields(record.screen).map((f) => f.label || f.name).join(", ")}</p>}
          ...
          <table>
            ...
            <tbody>{spec.screens.map((s) => (
              <tr key={s.id} style={s.hidden ? { opacity: 0.5 } : undefined}>
                ...
                <td><input value={s.title || ""} disabled={readOnly} onChange={(e) => updateScreen(s.id, { title: e.target.value, title_manual: true })} style={{ width: 150 }} /></td>
                <td><select value={s.kind} disabled={readOnly} onChange={(e) => updateScreen(s.id, { kind: e.target.value, nav: ["list", "form", "detail"].includes(e.target.value) && !s.hidden })}>
                  {["list", "form", "detail", "action", "other"].map((k) => <option key={k} value={k}>{k}</option>)}</select></td>
                <td>
                  <select value={s.table || ""} disabled={readOnly} onChange={(e) => updateScreen(s.id, { table: e.target.value || null, table_manual: true, table_source: "choisi à la main", table_confidence: 1 })}>
                    <option value="">— aucune —</option>
                    {readOnly && s.table && !(spec.tables || {})[s.table] && <option value={s.table}>{s.table}</option>}
