        ...
        <>
          {!l.connected ? <p><T tone="bad">non associé</T> <span className="muted">— l'interface {d.iface || "Wi-Fi"} n'est rattachée à aucune borne : aucune mesure d'expérience possible.</span></p> : (
            <div className="sa-kv">
              ...
              {kv("Occupation canal", d.channel ? <><T tone={pctTone(d.channel.busy_pct, 60, 80)}>{fmt(d.channel.busy_pct, " %", 1)}</T><span className="muted"> · par d'autres {fmt(d.channel.other_pct, " %", 1)} · nous ↑{fmt(d.channel.tx_pct, " %", 0)} ↓{fmt(d.channel.rx_pct, " %", 0)}{d.channel.delta ? "" : " (cumul)"}</span></> : <span className="muted">—</span>)}
              ...
              {kv("Notre borne (balises)", d.neighbourhood?.our_bss ? <>{d.neighbourhood.our_bss.stations ?? "?"} stations · utilisation annoncée <T tone={pctTone(d.neighbourhood.our_bss.utilisation_pct, 60, 80)}>{fmt(d.neighbourhood.our_bss.utilisation_pct, " %", 0)}</T></> : <span className="muted">{d.neighbourhood?.error || "—"}</span>)}
              {kv("Voisinage", d.neighbourhood && !d.neighbourhood.error ? <>{d.neighbourhood.bss_count} bornes · canaux {Object.entries(d.neighbourhood.channels || {}).map(([c, n]) => `${c}×${n}`).join(", ")}{d.neighbourhood.co_channel?.length > 0 && <span className="muted"> · co-canal : {d.neighbourhood.co_channel.map((b) => `${b.ssid || "?"} ${fmt(b.signal_dbm, " dBm")}`).join(", ")}</span>}{d.neighbourhood.same_ssid?.length > 0 && <span className="muted"> · même SSID : {d.neighbourhood.same_ssid.map((b) => `canal ${b.channel} ${fmt(b.signal_dbm, " dBm")}${b.stations != null ? ` (${b.stations} st.)` : ""}`).join(", ")}</span>}</> : <span className="muted">—</span>)}
            </div>
          )}
          {alerts.length > 0 && (
